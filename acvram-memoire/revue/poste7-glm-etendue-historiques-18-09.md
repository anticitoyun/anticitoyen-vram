# poste7 — Étendues > 4096 sur les GLM historiques : pas un chantier, un contrôle de 30 min ; hypothèse = experts à 1-7 jetons sur le corpus six phrases, avant `MIN_ECHANTILLONS_AWQ` (18/09)

Entrée : chef — scan poste2 118/118 ; GLM (SiLU à porte, sans ReLU²) : `srcbf16-nvfp4` 1 tenseur 81 391 ; `avant-echelle-fix`/`avant-mtp-fix`/`avant-noawq-experts`/`k48` 14 tenseurs chacun, max 16 457, même tenseur récurrent couche 46 expert 42 ; `k48-w4a4` 52 (17 875) ; `k48-calibA-verif17-09`/`k48-calibB` 3 chacun (5 188-16 723). **`k48-calibA` classé : 0.** poste2 sans hypothèse.
...
Convertis historiques à étendue > 4096 : non servis, hors menus, listés avec sha256 dans le verdict de poste2 ; suppression du disque = décision utilisateur.

## Ordre

* poste2 : contrôle § 3 (n_samples des 14 tenseurs de k48 sur le corpus six phrases + date de conversion des 8 convertis) → `verdict: revue/verdict-glm-etendue-historiques-18-09.md — <14/14 ou n/14>` ; une ligne sur le manifeste des repliés.
* chef : rien à republier ; chantier non ouvert sauf verdict < 14/14. Le scan du parc est clos.
