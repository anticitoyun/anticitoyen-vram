...
- **instrument** : `scratchpad/reppl-eval-16-09.py` = `acvram.evaluate.perplexity` (ligne de Manon : wiki-gptq, 2048/2048, min-context 256, 4 fenêtres) + régime lu après chargement ; `ACVRAM_QA_COMPTE=1` ; `seconds` = chargement + 4 fenêtres (`evaluate.py:175/364`) ; sorties `scratchpad/reppl-sage7-16-09/`
- **commit** : arbre **travail/laure-qa @ 877169c** (laurine, échelle globale par ligne, Sage § 7) ; t-qa 202/206 (4 rouges hors chemin : attente `0xFE` = −448, référence float64 du fused) ; protocole 6c91a5b
...
- **scellé** (Sage § 7) : B alpha-commun ≤ 1,010, 0 saturé, flush ≤ 0,01 %, durée ≤ 7,2 s, réfuté > 1,015 → sonde par étape avec Océane ; K `-k48` ≤ 1,005, réfuté > 1,010 → métrique W4A4 cb2784b. Moi : B 1,012 ± 0,004 ; flush 0,001-0,01 % ; 25 s = cache (K1 lent, K2 rapide) ; K 1,003-1,008
- **mesuré** : B **8,368142 = 1,02769** (×2 identique), flushés **0**, saturés **0**, 25,5 s puis **6,3 s** ; K **8,291296 = 1,01825** (×2 identique), 0/0, 5,9 / 5,9 s
- **verdict** : compteurs et durée **tenus** (0/0, 6,3 s ≤ 7,2) ; PPL **RÉFUTÉE deux fois** : alpha-commun 1,0277 > 1,015 → sonde par étape (Océane) ; -k48 1,0183 > 1,010 → cb2784b entre. Mes prédictions 1 et 4 réfutées, 2 et 3 tenues (K1 rapide parce que `-k48` venait d'être écrit à 08:08 : déjà en cache)
...
alpha-commun    W4A16 (MMA=0)              8,215    1,0089   —            —        Manon, mma0-confondant
...
Rien ne bouge sur la carte : sonde par étape (Océane + Laurine) sur l'alpha-commun, et la question « W4A4 au décodage à ce prix de PPL » revient à Sage avant le pas b=12 et le duel (le duel W4A16 reste possible : livrable 0,998, régime à vérifier).
