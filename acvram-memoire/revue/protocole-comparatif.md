...
Soumis par Jérôme le 8 septembre, 22h30. **C'est le chantier qui mesure
...
**L'erreur va contre nous** (Océane) : nos 122 t/s incluent le +1,5 % de
...
**Océane** — les appariements. Y a-t-il un réglage de llama.cpp qui rendrait la
...
**Manon** — l'énergie. Comment l'intégrer proprement plutôt que deux relevés ?
Et le banc mesure-t-il la même chose des deux côtés — même définition du débit,
même exclusion du TTFT ?

**Laurine** — quand tu auras fini la garde : éprouve que `--speculative none`
...
## Le modèle du comparatif — proposition de Jérôme, à valider

Océane a écarté à juste titre le `Qwen3-0.6B-Q8_0` qui a produit les premiers
...
* le contexte apparié — `llama-bench` a rendu ses chiffres à `n_ctx 256`, qui
  est son **défaut, pas un choix** (Océane). Que fait acvram par défaut, et
...
## `n_ctx` et le choix du modèle — tranché par Océane
...
Océane exigeait qu'on le nomme avant de convertir. Calculé sur la configuration
...
**Le risque qu'Océane refusait de laisser implicite est donc levé par le
...
Décision d'Océane, adoptée sans réserve : **ne pas aligner un défaut sur un
...
### Quatre asymétries à fixer avant de lancer (Manon)
...
**Et ce choix rend valide la trouvaille que Jérôme vient de retirer** : son relevé
