...
3. **1aj commence maintenant, à sec, chez Laurine**, en parallèle de la file — elle ne prend la carte qu'après P1. `bt=32` (2) sort de la file : sans objet chiffré.
4. **1aj touche le format du dossier** (projections int8 promu → NVFP4) : la PPL est le premier seuil, pas la vitesse. Scellé : Coder-30B, PPL ≤ étalon int8-promu × **1,01** (réfuté > 1,015 → 1aj se limite à `lm_head`, −0,15 Go) ; b=1 J/jeton **≤ 1,10** (−20 % sur 1,377 ; réfuté ≥ 1,25) ; ms ≤ 3,6 (seuil de Laurine, gardé). Attention : QKV est le tenseur le plus sensible à W4 (`duck-manon-12-09`, +0,012 KLD) — l'issue « la PPL refuse » est nommée, et elle est la plus probable pour q/k.
...
A7 (Manon, 45,3 dB, ×1,92, 32/32 fusions) : PPL après Laure/Laurine, inchangé.
