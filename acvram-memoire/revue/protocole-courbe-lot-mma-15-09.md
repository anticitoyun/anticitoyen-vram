...
[`sage-mma-lot-15-09.md`](sage-mma-lot-15-09.md), transmis par Jérôme.
Attend v0.6.2 de Laurine (`ACVRAM_MOE_DECODE_MMA_MIN_T`) sur main et la
carte après Manon.

## Critère de Sage (scellé)

MMA retenu au lot `t` dès que **J/jeton(MMA) ≤ J/jeton(GEMV)** ET
**ms(MMA) ≤ 1,02 × ms(GEMV)** ; défaut = le plus petit `t` qui satisfait
les deux. Prédictions de Sage (ms, relatif B/A) : b=2 **+20/+30 %**, b=3
...
Coût fixe de B lu par Sage : 1,54 ms/pas. Pas A attendus (rondes ctx
...
+9 % (b=3), +1 % (b=4), −7 % (b=6)** — dans les fourchettes de Sage sauf
