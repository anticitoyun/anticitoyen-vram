...
instrument : `scratchpad/s2-contexte-20-09/chaine.sh PRISE=4` (arbre pinné `manon-qvl` = origin/main 2919dd3c, `cd "$L"` avant `python -m`), en-tête sans pair (charge 2,60, gpg/glab/python3)
scellé (Maîtresse, avant) : 0 × 500 sur les 4 ; k48 doit charger et tenir 31744 ; i8c ≥ 12288, sinon écart nommé
...
durée : 09:48:20-09:55:24 (7,1 min) ; HEAD `manon-qvl` 2919dd3c
suite : Maîtresse : trancher l'écart i8c (11264 vs 12288) ; § 1b (commande exacte), GLM b=1 (charge < 1), NVTX Laurine (après 10 h 10) — non joués, la fenêtre se ferme ici

## Rejouable
`PRISE=4 ACVRAM_ARBRE=<arbre pinné> MANON=<manon> PYA=<venv>/bin/python ACVRAM_MODELES=/mnt/AI_GENERATOR/models_acvram CPUS=0-15 bash <manon>/scratchpad/s2-contexte-20-09/chaine.sh` (≈ 7 min).
