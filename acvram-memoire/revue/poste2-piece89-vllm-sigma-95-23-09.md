# Pièce 89 — comparatif vLLM à ≥5 lots/bras, ABBA, σ + IC95 %, seuil 2σ (poste2, 23/09)

instrument : `scratchpad/poste2-p78-23-09/run.sh` (X = acvram serve défaut, Y = vllm serve
  --attention-backend TRITON_ATTN, -lgc 2700 explicite autour de chaque bras Y), adapté en
  `scratchpad/poste2-p89-23-09/bloc.sh <b> <ABBA|ABAB>` avec BANC_SLOTS=b,b,b,b,b (5 répétitions du
...
scellé : écart déclaré seulement si |moyenne_acvram − moyenne_vllm| > 2·σ_diff (Welch, n=10 par moteur =
  ...
  (`scratchpad/poste2-p89-23-09/scelle.md`, avant la prise)
mesuré :
...
verdict : les DEUX écarts de vitesse (b=12 vLLM devant, b=1 acvram devant) passent le seuil 2σ —
  ...
  explicite cette fois). Contrôle poste3 (commit cb7c4147, 12h05:52) : tombe dans la fenêtre du PREMIER
