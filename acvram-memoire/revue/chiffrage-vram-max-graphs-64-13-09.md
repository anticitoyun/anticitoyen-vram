# Chiffrage — VRAM de `MAX_GRAPHS` 16→64 (item A9, audit Sage)

Laure, 13/09/2026, à sec (en attente de carte, occupée par Laurine). Suite à
`sage-audit-connaissances-14-09.md` item A9 : `MAX_GRAPHS` 16→64 réduit la
dispersion (65,11 % → 20,37 %, `laurine.md:4040-4082`) mais son coût VRAM
n'a jamais été chiffré. Critère de Sage : `< 25 % de dispersion à ≤ +1 Gio
→ adopter 64`.
...
être confirmé par la manche de 1 h / 10 modèles déjà prévue par Sage, pas
...
**Dans les deux cas, très largement sous le seuil de 1 Gio de Sage** — y
...
gigaoctets** — largement sous le seuil de 1 Gio fixé par Sage, dans
