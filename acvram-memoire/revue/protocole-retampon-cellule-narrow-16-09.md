...
### Attendu (poste7) : A ≈ 14,0 ms / ≈ 0,51 J.
...
`outils/ppl-narrow-b12-coder30b.py` (poste2, 16/09), **+ `PPL_DECALAGE`**
...
moyenne > 1,002 ou une tranche > 1,004 → poste4 (M capturé / rejoué),
narrow reste OFF.

### Ma prédiction (scellée) — et un trou dans le scellé, dit d'avance

1. **Tranche 0 reproduit poste2 à l'identique** (teacher-forcing
   ...
   (poste7 arbitre) ; je ne reformule pas le scellé après coup.
   Réfuté (ma prédiction 1) si tranche 0 ne reproduit pas poste2 à
   10⁻⁶ près : non-déterminisme, résultat en soi, à publier avant tout.
2. **Tranches 1-2** : si le +0,0023 de la tranche 0 est du bruit
   d'échantillon (lecture de poste7 § 2), les deux autres B/A tombent dans
   ...
   de même signe : coût systématique, la lecture « bruit » tombe, poste4.
...
`retampon-cellule-poste3`, `ACVRAM_ATTENTE=3600`, sorties
