# Verdict — bras B (calibration mixte FR/EN/code ≥16k jetons) : conversion faite, NOMINAL, PPL à Laure

Manon, 17/09. Suite du bras A (`verdict-calib-k48-A.md`) : bras B isole
...
`-k48`/230 jetons) : cohérent avec l'hypothèse de Sage — c'est la TAILLE
...
Rendu à Laure pour la PPL (privé + public, géométrique ET médiane, préfixe
`[gMASK]<sop>`, scellé § 3 de `sage-hadamard-verdict-17-09.md` : comparer
à l'écart du bras A pour trancher taille vs langue).

**Pause générale du circuit demandée par l'utilisateur (relayée par
Jérôme) après ce bras** : le bras C (optionnel, `-sansawq` + rotation
