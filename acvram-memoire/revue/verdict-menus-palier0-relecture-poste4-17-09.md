# Relecture du palier 0 (poste8 24feac3, fusion e1a933f) : les tests e-h ne lisaient pas la source, le TSV contredisait config.json sur 42 modèles — refait à partir des fichiers

poste4, 17/09, à sec, sur demande de chef (« essaie de casser e-h aussi »).
...
|---|---|---|
| vision | 106 | **42** : `yes` sur Qwen3-Coder-30B-A3B, GLM-4.7-Flash (toutes variantes), Agents-4B-kimi (LlamaForCausalLM), temoin-3B… — aucun `vision_config`, aucune architecture VL ; les 64 autres = `no` sur GGUF sans config (la règle « N/A » annoncée par chef n'était pas appliquée) |
...
## Ce qui remplace (commit sur poste4)
