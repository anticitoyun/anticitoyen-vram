# GDN par flash-linear-attention — implémentation à sec (Laurine, 17/09), en attente de la fenêtre carte de Laure

Commande : sage-priorite-apres-campagne-17-09 § 2 (Qwen3.8-27B b=12 : 97 j/s
...
1. `flash-linear-attention 0.5.2` (+ fla-core, einops) installé dans `.venv`
   ...
   **JIT sm_120 non contrôlé à sec** (pas de carte) : c'est le premier
   point de la fenêtre de Laure (faux si l'import ou le JIT échoue).
2. `engine/gdn.py` :
...
- **bras cassant de Sage** : l'état passé à fla avec l'axe [K, V] inversé
...
- la ligne de régime d'un MOTEUR CHARGÉ à sec porte `ACVRAM_GDN=fla`
  ...
  `torch` sous `ACVRAM_GDN=torch` — le contrôle demandé par Jérôme ;
...
## Scellés de Sage, rappelés (fenêtre Laure ≤ 40 min, Qwen3.8-27B calibA)

b=12 97 → ≥ 400 j/s (faux si < 250 : profiler) ; prefill 2 048 hybrides
≥ 5× la voie torch ; PPL ± 0,002 ; b=1 : t/s ≥ 85 ET J/jeton ≤ 0,75 × calibA
(63,8 t/s, dénominateur corrigé par Sage) — si W plafonne des deux côtés,
