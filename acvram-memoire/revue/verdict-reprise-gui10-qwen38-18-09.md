...
commit : dfc0bdb (laure = main 79ac659), 09:47-09:51, régime classé, défauts RPW=4 + XREG=down (rien de posé), graphes on
...
## Notes
- `/metrics.energie` ne contient que `j_par_jeton_10s` et `cartes` : les champs `fenetre_s` et `jetons_fenetre` annoncés dans la décision (sage-metrics-energie-fenetre-18-09) ne sont pas dans app.py:836-841 — à Jérôme de dire s'ils sont attendus.
