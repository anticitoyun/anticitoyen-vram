# Verdict — duel GLM-4.7-Flash prise A : vLLM NVFP4 = 4,1 × notre débit à b=12 (796 contre 195 t/s), 0,28 × notre J/jeton ; scellé de Sage (0,7-1,3×) RÉFUTÉ, ma prédiction (1,3-1,9×) RÉFUTÉE
...
- **commit** : travail/laure **42c24fb** (code `acvram/` = main ce71723) ; protocoles 9998eca + 2cb4e22 + f07ece8 ; vLLM 0.29.0, `/opt/ia/vLLM/.venv`
...
- **scellé** : Sage `sage-objectif-14-09` : vLLM 0,7-1,3 × notre débit b=12. Moi (f07ece8) : nous 205-222 t/s b=12, vLLM 1,3-1,9 × ; b=1 22-27 ms ; prefill acvram 8-12 k j/s > vLLM 6-10 k ; fp8 dyn OOM
...
```
Le décodage MMA W4A4 (godet 12) n'est pas couvert par la PPL de prefill ; le coût E2M1 y reste (Sage § 8.3).
...
Prise B (ncu octets par jeton, `campagne-duel-glm-ncu-15-09.sh`) dira si vLLM lit moins d'octets ou les lit plus vite ; PPL vLLM même corpus (10 min) pour la colonne ; un nsys d'un pas b=12 GLM (`nsys-rejeu-b12-15-09.py`, 5 min) pour découper les 56 ms en MLA / MoE / trous — c'est la mesure qui nomme le chantier. Je ne lance rien : Sage tranche l'ordre.


## Réserve (Sage § 6, 16/09)

bras vLLM GadflyII à +5,4 points de PPL (1,056), hors seuil 1,02, non apparié : sa vitesse ne se classe pas — sans en faire une excuse, mêmes octets NVFP4 des deux côtés, notre ×4 est réel et se lit dans 15 534 lancements/pas. Bras vLLM apparié = conversion ModelOpt par nous (Manon, dans le comparatif 5 moteurs, pas avant).

## ERRATUM FORMEL (16/09, Sage § 10, cause : verdict-glm-slots12-erratum-16-09)
...
## PUBLICATION FINALE (16/09, Sage § 14, après correctif tête fp32 et prise B)
...
clos. Postes restants (Laurine, Manon) : projections int8, tête GEMV, MoE MMA.
