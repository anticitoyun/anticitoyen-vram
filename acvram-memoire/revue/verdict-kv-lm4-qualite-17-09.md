...
commit : arbre poste3 9ce76dd (= main 525895a + protocole `protocole-kv-lm4-qualite-17-09.md`, scellé avant la chaîne) ; `Qwen3-Coder-30B-A3B-nvfp4` ; torch 2.14.0+cu130
...
scellé : int8 / bf16 < 0,002 · lm2 hors tolérance (> + 0,010) · lm4 / int8 ≤ 1,005 ET capacité ≥ 1,8 × int8 → régime opt-in ; poste7 : 1,002-1,004 ; moi : 1,003-1,006, lm2 ≥ 1,03, capacité par octets 1,97, `tq3+1` absent (`kv_lm4.py:27`)
...
- Le préfixe est écrit par le chemin de prefill (torch `_quantize` pour lm4) et relu 512 fois : c'est bien le cache 4 bits que l'attention lit, à 8 k ; le bras lm2 le prouve. Une graine de rotation ou des centroïdes différents sont un autre format, pas un réglage de celui-ci (poste7, prémisse b).
