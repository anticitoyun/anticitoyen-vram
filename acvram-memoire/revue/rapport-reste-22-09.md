# Ce qui reste avant la fin du projet — 22/09 15 h 3x (chef)
...
A. Mesures en cours (poste2, aujourd'hui) : 5e chaîne énergie 4 moteurs (vLLM et TRT-LLM manquent encore), rejeu invite4 du KL, diag-eval-nll 31B (2 min) ; poste3 : KL TRT-LLM 4 invites (≤ 10 min) ; preuve du hang de capture (≤ 5 min).
B. Combler b=12 (poste1, une pièce par heure en régime mesuré) : 35 occupation des étroites (ptxas -v, balayage BLOCK_M/num_warps/num_stages au bit ; prédit ≥ 10 % sur `o`) → 39 fusion tête + échantillonnage (nsys d'abord, ≤ 3 %, fermée si < 2 %) → 30 cellule kv-fp8 (poste3, jugée par débit + KL). Fermés sans gain : split-K (33), PDL (34), Marlin isolé.
...
F. Régime : hebdo à 78 % (réserve) jusqu'à samedi 15 h ; poste1 et poste3 n'avancent B que par exceptions ≤ 15 min ; le gros de B se fera après la remise hebdo.
