# H2 (2) — table site → nœuds → ms/pas, à sec depuis le sqlite existant (0 min de carte) : les 336 lancements ne sont PAS de la glue MoE par couche, ce sont des noyaux à la FRONTIÈRE DE PAS (poste4, 21/09)

instrument : `scratchpad/poste4-b12-21-09/ordre-noyaux.py`, sqlite déjà exporté (`nvtx-h2.sqlite`, prise de poste2 09:15-09:21), aucune nouvelle carte
...
Ma note du 21/09 09h31 cherchait une fusion « glue par couche » qui n'existe pas sous cette forme : **il n'y a pas de nœuds MoE non attribués par couche** (les 13 lancements/couche sont déjà tous nommés, dont `_route_fusee`/`rmsnorm`/`Kernel2` déjà comptés dans le budget nsys initial). **H2 devient : le poste de 0,43 ms/pas de la tête + échantillonnage, déjà repéré par H1 comme le « trou »** — ce n'est pas un chantier séparé, c'est la même pièce. Aucune fusion à proposer pour poste1 sur une « glue MoE » : elle n'existe pas dans ce régime.
...
suite : chef : (1) H2 fermé — pas de glue MoE à fusionner ; (2) le poste 0,43 ms rejoint H1 (pipeline en service) : poste1 peut viser directement `_sample_only`/`_consommer` (argmax device déjà en place selon `graphes-preparer-divergence-14-09`, mais softmax/amax `[12,151936]` fp32 en pleine précision pourrait se réduire à bf16/torch.compile) au lieu d'une piste MoE séparée ; (3) confirmation encore utile mais non bloquante : LISEZ-MOI v2 eager (envoyé à poste2) donnera les vrais noms de sites via NVTX, pas seulement l'ordre — à jouer sans urgence, cette note suffit pour trancher H2 close.

## Rejouable
`python3 scratchpad/poste4-b12-21-09/ordre-noyaux.py scratchpad/poste4-b12-21-09/sorties/nvtx-h2.sqlite`
