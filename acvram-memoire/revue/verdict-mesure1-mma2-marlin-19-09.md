# Verdict — Mesure 1 à unité égale (Coder b=12, même liste de 96 lignes, 45 puis 27 experts distincts) : mma2 est **plus rapide** que Marlin à octets égaux (×0,88 à 45, ×0,75 à 27) mais **au même plafond de 400 W** (401 W ≥ 380 : réfutation de Sage atteinte) → **Mesure 2 : NON** tel que scellé (`sage-c16bis` § 2 : t ≤ 1,15 t_marlin TENU, W ≤ 350 FAUX) ; la règle de l'addendum 20 h 15 (W·t ≤ 0,85·398·t_marlin) rend faux de 4 % à 45 et tenu à 27 — Sage tranche

instrument : `outils/energie_par_poste.py 12 6` mode `BANC_SEULEMENT=mesure1` (oceane-11 après 7cc9d5fa, ce commit) — 5 noyaux sur la MÊME liste d'experts (générateur figé : B=12 jetons × top_k 8 = 96 lignes, exactement u distincts, 8 distincts par jeton, `liste_experts`), 48 couches en tampons tournants (DRAM, pas L2), boucle soutenue 6 s par poste, W = compteur NVML, **t noyau** = Σ durées CUPTI (`torch.profiler`, passe séparée après la boucle) / lancements, rapport cyclique = t noyau / t mur, Go/s = octets d'expert (0,885 Mo par projection, lus dans les piles) × u / t noyau ; témoins copie DRAM 1 Gio (339-342 W, 1 405 µs = 0,76 Go/ms… 1,53 To/s) et GEMM bf16 8192³ (400 W). Chaîne `donnees-mesure1-19-09/chaine-mesure1.sh` (scellé en tête), journaux `journal-{marlin,naturel}.log`
commit : oceane-11 7cc9d5fa (arbre), 19:50-19:57, sous `carte.sh`, **deux processus** (les deux dispositions ensemble, `ACVRAM_DOUBLE_DISPOSITION_DIAG=1`, sortent de la mémoire : 24,1 Gio alloués + 6,5 réservés + **PID 4284 `llama-server` 5,6 Gio, pas à moi, hors verrou, présent avant et après**, laissé en place) — bras Marlin `[régime] NOMINAL … experts_layout=marlin` (défaut), bras naturel `ACVRAM_GEMV_LAYOUT=naturel ACVRAM_PREFILL_GROUPED=groupe … experts_layout=naturel` ; repos 48,8 W (bras 1) / 63,1 W (bras 2) contre 34,5 le matin — le processus étranger ; les W publiés sont absolus (c'est ce que la règle compare à 350), W_net dans les journaux
...
scellé (Sage `sage-c16bis-puissance-mesure1-19-09` § 2, en tête de la chaîne avant la mesure) : Mesure 2 seulement si t_mma2(45) ≤ 1,15 × t_marlin(45) ET W_mma2 ≤ 350 ; t_mma2 = gate+up + down + quant_act ×2, t_marlin = gate·up + down ; prédictions Sage : Marlin 78 µs / 398 W / 1,5 To/s, mma2 90-125 µs / 280-340 W / 1,0-1,3 To/s ; réfutation Sage : W_mma2 ≥ 380 ; prédiction Océane : t_mma2/t_marlin 1,1-1,4 à 45 ; contrôle : rapport cyclique ≥ 0,9 par noyau, alarme Marlin gate·up hors ± 15 % de 398 W
mesuré :
...
* Le processus étranger 4284 (llama-server, 5,6 Gio) était sur la carte et son repos diffère entre les deux passes (48,8 W bras Marlin, 63,1 W bras naturel, contre 34,5 W carte vide le matin) : les W absolus des deux bras ne sont comparables qu'à ± 15 W. Ramenés à un repos propre (W_net + 34,5) : **mma2 370-373 W, Marlin 384 W** — la réfutation de Sage « ≥ 380 » tiendrait alors de 7-10 W pour mma2 (non tranché à cet instrument), le seuil « ≤ 350 » reste faux dans tous les cas, donc Mesure 2 reste NON. Une remesure carte vide (3 min) lève le 372/401, elle ne change pas le verdict.
* Unité : 45 et 27 sont des listes construites (8 distincts par jeton), pas le routage réel ; le compte réel par couche à b=12 vient de la trace C9-M1 (Manon, prédiction Sage 30 ± 8).
...
## Mesure 1-bis (20:10-20:13, `donnees-mesure1-19-09/chaine-mesure1bis.sh`, u = 8 à b=1 et u = 16 à b=2, 4 s par poste, sans témoins) : **Marlin garde b=1 et b=2** (prédiction Sage tenue)
...
Par couche : u=45 Marlin 110,9 µs, mma2 99,0 (**×0,893**) ; u=27 88,8 contre 68,5 (**×0,771**). Scellé Sage (`sage-c17-scelle-mesure1-ter-20-09`, prédiction 0,95-1,04 : **réfutée** ; C17 s'écrit si ≤ 0,92 : **tenu**) ; ma prédiction 0,93-0,98 réfutée aussi. Mon contrôle « MHz des quatre noyaux à 2 640-2 700, sinon éco non effectif, non publiable » rend **faux pour Marlin (1 942-2 265 MHz)** — mais la cause est physique et vérifiée dans le même processus : l'éco est effectif (2 685 lu sous charge légère, mma2 à 2 692 sous 390 W), c'est **le plafond de puissance qui tire Marlin sous le verrou** (398-404 W à 2 000 MHz : ses instructions par octet le rendent co-limité, `c16bis` § 2) ; `-lgc` ne peut pas tenir une horloge que le bridage refuse. Lecture : sous le défaut servi (éco 2 700), l'écart mma2/Marlin n'est PAS un écart d'horloges qu'on égalise, c'est l'écart de ce que chaque noyau obtient sous 400 W ; le −11 % (u=45) est le chiffre du régime servi. Sage tranche C17 avec ce chiffre ; mma2 à u=27 tombe à 358 W (sous le plafond) : le seul poste qui rend des watts.
