# Note — la classe Marlin (vLLM v0.29.0, `csrc/libtorch_stable/quantization/marlin`, `moe/marlin_moe_wna16`, Apache-2.0) appliquée à notre pile : faisable en 3-5 j **oui, comme PORT** ; porte 110 TFLOPS **corrigée à 137** (Laurine, 18/09, à sec, aucune mesure)
...
- `outils/banc-marlin-p1-18-09.py` : formes Coder 2 048 (E 128, top_k 8,
  ...
  (classe B0), X = 7,43 TFLOP / (ms × 48), trois bandes de Sage imprimées,
...
### Conditions de Sage intégrées au banc (18/09, à sec)
...
### Prédiction GLM par la formule (écrite avant la passe de Laure, 18/09)
...
## Intégration P1 (18/09, après X = 152,1 TFLOPS au banc de Laure — bande haute)
...
- `MoEBlock._construire_marlin` (à la construction des piles) : seconde
  ...
  tort (Sage/Jérôme 18/09), levée. Refus explicites, une ligne : piles non
...
Scellé in situ (Laure, harnais égal, Coder 2 048) : **≥ 15 700 j/s**
...
## Arrêt de Laure (5803c1e) — trois défauts, deux ordres de Sage, verdict à sec
...
3. **L'écart 3,7-4,3 % hors 2⁻⁷·(moy|y|+|y|) à T = 1024 — hypothèse (a), le
   ...
   - (a) critère : émulation torch (CPU, sans noyau) de B0 (déquant bf16 :
     ...
     sous le critère relatif de Laure, **B0 lui-même 2,4 % hors, Marlin 0,6 %,
     Marlin-contre-B0 3,0 %** — c'est le chiffre de Laure (3,7-4,3 %) :
...
PPL préfill = B0 ± 0,002 reste le juge final (Laure, avec la vitesse) ; ma
prédiction : Marlin ≤ B0 en PPL (plus proche de fp32), écart < 0,001.

### Deux corrections de test (Sage, un commit, à sec)
...
- [1024-awq] : hypothèse 1 de Sage tenue — la référence recalculait l'AWQ en
...
## Disposition unique (sage-p1-disposition-unique-18-09) : banc du décodage, à sec

Sage tranche : UNE disposition Marlin, servie aussi au décodage (le chemin
...
- `outils/banc-marlin-decode-18-09.py` : b=12 (96 paires) et b=1 (8 paires),
  ...
  disponibles à sec — distincts imprimés), bandes de Sage imprimées.
Prédiction, la mienne (Sage : b=12 4,6-5,4 ms, b=1 0,9-1,1) : b=12 **5,0-6,0
...
### Résultat du banc décodage (Laure) : b=12 **6,81 ms/pas**, b=1 **1,41 ms** — hors bandes (> 6,5 et > 1,2)

Ma prédiction (5,0-6,0 / 1,1-1,4) : b=1 tenue, b=12 dépassée de 0,8 ; celle de
Sage (4,6-5,4 / 0,9-1,1) fausse des deux côtés. Marlin à M = 8-12 par expert
...
scalar type Byte ») — vues uint8 portées dans le fichier (Laure avait rejoué
depuis une copie locale). Suite : décision de Sage — forme (b), GEMV relisant
