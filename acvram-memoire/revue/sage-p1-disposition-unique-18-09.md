...
Laurine, 2 j : `nvfp4_gemv_grouped_*` lit les quartets dans l'ordre des fragments Marlin (table de permutation constante, 16 valeurs par tuile) ; le GEMV vient d'être réglé (rpw/xreg) sur l'ordre naturel, donc re-porte ncu-borné + ABAB ; scellé : b=12 nu ≥ 0,97 × 1 312, b=1 inchangé ± 3 %, bit-exact contre le GEMV actuel (mêmes codes, même ordre de somme). Si (b) échoue aussi : **P1 fermé pour VRAM sur Coder**, verdict daté, et la ligne utilisateur le dit — pas de disposition hôte, pas de repack.

## Ordre

* Laurine : banc § 3 à sec (formes décodage, routages réels, aligneur capturable) ; prédiction écrite ; puis Laure 20 min. Rien d'autre sur P1 avant ce verdict.
* Jérôme : ETAT — P1 « bloqué VRAM, voie disposition unique, banc décodage en cours » ; carnet Sage réfuté (+1,7 Go) ; REGLES § 3 ligne « chiffre de VRAM = manifeste, pas une note ».
* Manon : P2 converti inchangé (indépendant). Océane, Katy : inchangés.

## 5. Verdict banc décodage (Laure ee7f76e) : b=12 Marlin 6,81 contre GEMV 6,92 (parité, 0 hors 2⁻⁷) ; b=1 Marlin 1,41 contre 0,82 (×1,7, structurel : tuile m16n8k16 pour une ligne) — forme (a) morte à b=1 ; forme (b) lancée, avec une variante (b') à chiffrer d'abord
