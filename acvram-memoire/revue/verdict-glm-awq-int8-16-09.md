...
poste1, à sec (aucun GPU), ordre poste7 §5
(`revue/poste7-glm-pile-correctif-16-09.md`, main f127a2e).

**Note sur le seuil transmis** : le message relayé (chef) donnait
« err(sans)/err(avec) ≥ 0,9 ». La note originale de poste7 (§5.3) écrit
...
poste7 : « erreur déjà ~0,3 % » — int8 par groupe est intrinsèquement
...
2 % d'une erreur déjà minuscule : la recette 2 de poste7 s'applique — AWQ
sur les tenseurs NVFP4 seulement, aucune table sur les projections int8
pour la reconversion de poste2. Le scellé du pas reconverti reste à une
seule variable (correctif W4A4 down_proj).

## Conséquence

poste2 : reconversion avec `--no-awq` restreint aux projections int8
(experts et dense en nvfp4 gardent l'AWQ). Scellé du pas (poste7 §5.4) :
