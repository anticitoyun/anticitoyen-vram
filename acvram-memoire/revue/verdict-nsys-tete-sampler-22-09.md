# nsys-tete-sampler (pièce 39) — ÉCHEC instrument, regex TÊTE mal ciblée — 22/09 (poste2)

* instrument : `outils/gpu/mesure/nsys-tete-sampler.sh` (poste1, 22/09) → `part-tete-sampler.py`, b=12, 60 pas, alias `Qwen3-Coder-30B-A3B-nvfp4`
* commit : main à jour
...
* verdict : **ÉCHEC — la porte de la pièce 39 (tête+sampler < 2 % du pas) n'est PAS jugeable sur ce résultat** : 94,04 % combiné serait absurde (aucun modèle ne passe 94 % de son pas dans l'échantillonnage). Chiffre non publié comme mesure réelle. Cause nommée pour poste1 : vérifier que `TETE` matche le DERNIER GEMM avant la fin du pas, pas le premier motif rencontré (ou resserrer le motif pour exclure les GEMM MoE/routeur qui peuvent partager `cutlass`).
* durée : ~5 min de carte, log gardé (`scratchpad/nsys-tete-sampler-22-09/part.txt`)

## Suite
Correctif à poste1 (regex ou logique de sélection du dernier GEMM candidat, pas le premier). Je rejoue --ptxas (corrigé, main 02a5f65a) puis diag-eval-nll (pièce 37), rends la carte à poste3 ensuite.
