# poste7 — Ordre de reprise (20/09, 19 h 36) — minimum de jetons, poste2 absente

Base : tête PAUSE d'ETAT 19 h 17 (main 2b8e387d) + `poste7-p3-1-scelle-temoin`, `poste7-3b-lanceur-contexte`, `poste7-auto-yes-efforts` (20/09). Faits intégrés : auto-yes a envoyé 2 tours « 1 » factices (diagnostic tenu) ; deux poste1 vivantes ; poste2 absente ; REGLES § 1 ligne 12 en cours (chef).

## Trois règles pour cette reprise

1. **Tant que le motif resserré n'est pas dans `/etc/auto-yes/patterns.conf`** (un `sudo cp` de l'utilisateur depuis le scratchpad de chef, après les contrôles ±) : aucune session n'affiche la phrase du motif — ni `cat`, ni Read de `poste7-auto-yes-efforts-20-09.md`, qui la contient cinq fois. Cette note-ci ne la contient pas et se copie sans risque. Les sessions vivantes gardent l'ancien motif jusqu'à relance : **on ne relance rien avant le `sudo cp`** (le `--resume` rejoue le transcript, c'est ce qui a tiré). Compte des tours factices tenu dans `chef.md` : 2 au départ ; chaque nouveau tour = une ligne datée.
2. **poste2 absente = aucune mesure GPU**, mais aucune chaîne non plus : `.qui` vide + `nvidia-smi --query-compute-apps` vide, relevés une fois dans le carnet de celui qui lance → les tests processeur (`rtk pytest` ciblé) passent sans « trou » à demander. Tout ce qui suit est à sec, sauf le bloc poste2.
3. **Doublon poste1** : chef écrit dans `chef.md` l'poste1 qu'il sert (son `[ref]` de ListAgents) ; l'autre ne lit, n'écrit, ne commite rien jusqu'à sa fermeture par l'utilisateur. Deux sessions sur la branche `poste1` = fusion cassée.
...
2. Addendum 19 h 03 : dtype des logits de `ref-2b-fp32.pt` et top-2 à 6 décimales sur img02/08/12/18 (1 min, à sec). Tous multiples de 0,125 → étage bf16 dans `references-transformers.py`, correctif + sha dans le pointeur ; sinon « fp32 plein, 0,750 = coïncidence » écrit tel quel. Ce pointeur débloque poste2 1.
3. Rien d'autre : pas de 30B, pas de qualité 31B avant 0.6.34.

## Ordre — poste9

1. Tests écrits non joués (lanceur source, verifier-contexte, paquet parc) : même règle qu'poste1 1.
...
3. § 3a : `verifier-contexte.py` garde la marque « plan » jusqu'au S2 rejoué (poste2 2) ; plans par lots après.
4. Xephyr 1a, VM parc : après 0.6.34 (feu vert parc 21/09 inchangé). Ni Chromium ni chef ce soir.

## Ordre — poste2 (dès l'icône lancée ; prise `carte.sh` ≤ 30 min)

1. P3 (1) : `chaine-p3-1-rejeu.sh` sur main ≥ 5d9de783, **après** le pointeur poste1 2 (refs réutilisées si sha égal ; kv=bf16 ×20 puis int8 ×20 ; 8 min) ; verdict 7 lignes, cinq clauses (b″), marges fp32 par pas.
...
Sans poste2, rien de 1-3 n'est fait par un autre poste.
