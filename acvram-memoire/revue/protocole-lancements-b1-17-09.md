...
commit : arbre poste3 (= main, C mixte défaut) ; régime classé `ACVRAM_MOE_MMA=0 ACVRAM_MOE_DECODE_MMA=0 ACVRAM_NARROW_GEMM=1`, défauts E/C/B0.
scellé (poste7) : ≥ 600 lancements par pas ; petits noyaux (glue MoE + norme/rope/act + élémentaire) ≥ 28 % du pas (1,17 ms / 30 % lus sur mon profil du 16/09 à 4,04 ms).
...
falsification : < 600 lancements → poste7 réfutée, F n'a pas sa cible ; petits < 28 % → idem ; un poste unique > 50 % des lancements → c'est lui la fusion à faire en premier.
