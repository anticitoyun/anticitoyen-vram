# Pièce 76 — vLLM retracé en service à -lgc 2700 ; familles de noyaux à horloge égale contre notre trace — 23/09 (Océane)

* **instrument** : chaîne `scratchpad/oceane-p76-23-09/chaine.sh` (celle de la p59, même client `decode-vllm-17-09.py`, b = 12, fenêtre 5 s, + -lgc 2700 et échantillonneur d'horloge 200 ms) ; `outils/gpu/mesure/familles-comparees.py` (taxonomie commune, **durées de noyaux seules**, pas = 48 marqueurs par couche, fenêtres établies ≤ 1,3 × mur médian) contre notre trace p73 (`scratchpad/oceane-p73-23-09/graphe_cuda_gpu_trace.csv`)
