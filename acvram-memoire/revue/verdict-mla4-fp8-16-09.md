...
- **commit** : arbre mesuré **travail/laure-qa @ f4607c2** (laurine ; test fp8 corrigé, noyau inchangé depuis 3a1d2fd) ; tests **25/25** ; bras **C** = défaut, **D** = `ACVRAM_MLA_LATENT_FP8=1`
- **régime** : `-k48`, prefill W4A16, décodage MMA=1 MIN_T=5, MLA_BATCH=2, une passe, prep ; graphes actifs (et eager en contrôle)
- **scellé** : Jérôme : PPL D/C dans 1 ± 0,004 (réfuté → bf16 gardé), mla_* −0,2 à −0,5 ms. Moi : mla_* 1,9-2,2, PPL D/C 1,001-1,004, top-1 ≥ 99 %, pas bit-identique
...
- **verdict** : bras D **non adoptable, et pas pour la raison scellée** : ni gain (0,0 ms à ctx ≈ 300 — la lecture du latent n'est pas ce qui borne `mla_1p` ici), ni coût de qualité mesurable — parce que **le contenu du cache latent ne change rien à la sortie du chemin batché** (E4M3 par ligne ≡ bf16 au 10⁻¹² sur 3 × 22 664 jetons : impossible si les codes étaient lus). Anomalie d'intégration, Laurine (§ 1). Mes prédictions : mla_* tenue (2,11), PPL et « pas bit-identique » réfutées — par l'anomalie, pas par le fp8
...
MoE W4A16 (MMA=0)              7,888561     —       l'instrument voit bien le décodage : W4A4 = +1,42 % au décodage GLM (chiffre pour Sage § 8.3)
```
