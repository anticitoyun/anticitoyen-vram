# diag-eval-nll, bras decode (pièce 37) sur Gemma 4 31B — INVALIDE, auto-détecté — 22/09 (poste2)

* instrument : `outils/carte.sh .venv/bin/python outils/gpu/mesure/diag-eval-nll.py gemma-4-31B-it-nvfp4-vision --jetons 300`, sha `bf8afbdc` (bras decode ajouté par poste1)
* commit : main à jour
...
## Suite
Bras decode à corriger par poste1 (le montage position 1 doit coïncider au bit près avec eval, contexte identique, aucun cache en jeu — l'écart de 0,0142 suggère un détail d'initialisation différent entre les deux chemins). P4/P5 restent non tranchés.
