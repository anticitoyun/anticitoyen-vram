...
Demandé par chef le 10/09/2026, en réponse à l'audit Go/s. Écrit par poste3,
**à exécuter dès qu'une carte se libère** — après poste4 et poste2.
...
6. Publier `Σ(temps nsys des noyaux non-poids) / t_mural du pas` à côté de `p`
   (demande d'poste1) : sans ce rapport la part n'est pas publiable ; avec lui
   c'est un rapport de deux quantités biaisées dans le même sens, donc
   utilisable.

**Deux contrôles miroir à vérifier AVANT de faire confiance au profil (poste1)**,
...
`outils/part-attention-trois-bras.py` (banc d'poste1, commit `3d66541`,
branche `poste1`) : une vérification qui peut arrêter la mesure avant qu'elle
...
(chef/poste1) — vérifiées par moi à l'instant, PAS encore présentes dans mon
arbre** :
...
* `~/.cache/acvram/kernels` est **partagé entre les quatre worktrees**
  ...
  annoncé (`ACVRAM_KERNEL_CACHE`) **n'est pas encore dans `origin/poste1`**
  (vérifié par `git show origin/poste1:acvram/kernels/__init__.py`, dernier
...
* **Publier le sha256 du `.so` chargé** dans le relevé — un `.so` peut être
  cohérent avec le `.cu` du mauvais arbre (poste2, ce matin).
* **Vérifier `acvram.__file__`** avant de mesurer quoi que ce soit — le
  ...
  coûté un essai à poste1.

**Fait qui déplace le sens d'une des deux issues, pas le protocole** : poste2 a
...
**Ordre d'exécution sur la carte, imposé par chef** : poste2 finit ses douze
mesures, PUIS poste1 compile ses trois branches en `nice -19` (~9 min, elle
signale la fin) — sa compilation ne peut pas cohabiter avec ce profil. **Ne pas
démarrer sur la seule libération de la carte : attendre le signal d'poste1.**

## 3. Le seuil — dérivé du chiffre contesté lui-même (poste1), pas choisi
...
**Ne pas transporter 42,9-49,5 %** (poste1 vient d'être corrigée sur ce point) :
...
**0,346 % est ~15× sous le seuil de 5,24 %** (poste1, corrigeant mon calcul
...
**Retiré le 10/09/2026 par poste1, avant toute mesure.** Tout ce qui suit dans
...
* Les **453 lancements non répartis** restent la vraie question — mais leur
  ...
  `nsys stats`, pas sur un temps** (poste1) : c'est un compte, non biaisé par
...
défini (§3, poste1) comme la part du temps prise par des noyaux qui **ne
...
(poste1, confirmé par moi à `acvram_kernels.cu:793,918-1924`)** :
...
code — lecture de profil seule. Pas de GPU sollicité avant que poste4 et
poste2 aient libéré la carte.
