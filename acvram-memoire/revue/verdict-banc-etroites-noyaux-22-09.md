# banc-etroites-noyaux (poste1) — RÉFUTÉ sur (b), (c) crash non mesuré — 22/09 (poste2)

* instrument : `outils/carte.sh .venv/bin/python outils/gpu/mesure/banc-etroites-noyaux.py --rep 200`, sha 9f57b402
* scellé (poste1) : (c) FP8 `_scaled_mm` 1,3-1,8 To/s ; (b) `_int_mm` 1,13 qkv / 0,67 o ; réfuté si aucun >1,0 To/s ; alarme >2,0
* mesuré :
...
## Suite
(c) `_scaled_mm` FP8 à corriger avant de pouvoir trancher (erreur de stride sur l'argument `b`, probablement une transposition manquante avant l'appel) — à poste1 si elle veut ce noyau testé.
