# Sage — stratégie 14/09 : rattraper vLLM à b=12, ou recentrer ?

Sources : audit-a2-duel-vllm-14-09.md, vis-a-vis-decodage-b12-14-09.md (laurine), prediction-4-3ms-hote-decodage-14-09.md, mesure-qui-tue-autopin-table-14-09.md, prediction-masquage-fantomes-14-09.md, verdict-cout-verification-ngram-b12-14-09.md (laure), audit-a1-ttft-profil-14-09.md, verdict-horloge-memoire-14-09.md, laurine.md:3736-3752,4335-4358, beads 0si/1aj/6wa, jerome.md. Aucun chiffre de moi : chacun renvoie à sa source. Ni carte, ni commit.
...
| hors rejeu | 826 interne → 717 HTTP (−13 %) ; 0,3 ms interne (Océane) | vLLM mesuré **hors ligne** | — | voie HTTP (2 `decode()`/jeton, SSE) ; régime à apparier d'abord | −1,5 | 1 |
...
**Prefill** (19 148 vs 34 788, ×1,82) : sur le pas L=2048 (~113 ms), `int8 dense` pèse 30 ms (laurine:4346) contre 3,3 ms de GEMM dense FP4 chez eux — c'est **1aj appliqué au prefill (M=2048)**, pas 0si, qui rapporte le plus : 30 → ~7 ms prédit, pas → ~90 ms, ~24 000 j/s ; 0si TMA ensuite (24 → 17 ms) → ~26 000. Reste ×1,3.
...
**Verdict (1)** : débit rattrapable à −15 % (1aj + lancements + HTTP, ~5 jours-sessions : Laurine 4, Manon 0,5, Laure 1) ; pas battable à b=12 sur ce modèle sans lire moins d'octets ; énergie : ne rien promettre avant instr/octet.
...
|---|---|---|---|---|---|
| a | **énergie brute à faible lot (b=1-4)** | repos 17 W (nous) vs 64 W (vLLM), même carte, instants différents (audit-a2, réserve) ; résident b=1 = 242 j/s : 47 W de repos = 0,19 J/jeton, soit tout le net de vLLM ; llama.cpp b=1 : 1 297 vs 1 432/1 161 j/kJ, indécidable (laurine:3755) | J/jeton **brut** b=1 : nous < vLLM d'au moins 30 % ; b=4 : ≥ 15 % ; vs llama.cpp : ≥ 10 % | même modèle, HTTP des deux côtés, carte exclusive, repos 30 s, compteur NVML, colonnes brut/net (duck-jerome), 10 ABBA, ≥ 512 jetons | repos vLLM ≤ 25 W isolé, ou brut vLLM ≤ nôtre à b=1 |
| b | **MLA NVFP4 sur sm_120** (GLM-4.7-Flash / GLM-42B) | ×1,40 vs concurrent après 6wa (54,38 ms b=12, laurine:4335) ; vLLM : FlashInfer absent, FA refuse KV fp8, MLA SM120 = FP8 seulement (sage-veille §2.4), aucun checkpoint NVFP4 GLM connu | vLLM ne sert pas GLM-4.7-Flash en NVFP4 sur cette carte, ou ≤ 0,8× notre débit b=12 avec 2-4× nos octets | `vllm serve` sur le checkpoint GLM le plus proche, b=1 et 12, t/s + J, octets lus/jeton publiés | vLLM ≥ nous à octets ≤ 1,5× les nôtres |
| c | **llama.cpp à b=12 sur MoE** | ×1,91 débit / ×1,43 J le 13/09 — binaire e34f042 (avril) ; Katy a reconstruit 4a899373 le 14/09 : noyaux MoE CUDA réécrits depuis | l'écart tombe à ×1,3-1,5 mais tient | même protocole que le 13/09, binaire 4a899373, Q4_K_M vs NVFP4, b=1/4/12 | ≤ ×1,1 → plus rien à revendiquer contre llama.cpp |
...
1. Énergie : instr/octet par noyau + J par poste, deux moteurs (1 j, Laurine ncu + Laure NVML) — décide si les noyaux sont le bon chantier.
2. 1aj W4A4 projections + `lm_head`, décodage **et** prefill (Laurine 3 j, Manon PPL 0,5 j).
3. Mesures 2a (brut b=1/4) et 2c (llama.cpp 4a899373) — les deux chiffres publiables de la semaine (Manon/Katy 1 j).
4. 2b MLA vs vLLM (1 j). 5. 0si TMA (2 j), après 1aj, pas avant.

## Correction 14/09 soir — 2a réfutée en ampleur à b=1, en sens à b=4

Source : `energie-brute-faible-lot-14-09.md:14-21` (Laure, branche `laure`, 42545de). Ma prédiction scellée en §2a — « b=1 : nous < vLLM d'au moins 30 % ; b=4 : ≥ 15 % » — est **fausse** : b=1 −3,8 % (sens juste, ampleur ×8 trop grande), b=4 **+20 %** (sens faux). Cause : la prémisse « repos 17 W vs 64 W » était un repos **froid** (rien chargé, audit-a2) appliqué à un régime chaud ; mesuré modèle chargé : 68-76 W nous, 89-97 W vLLM. Règle 4, « chiffre exact hors de son régime » — la mienne.

Ce que le tableau de Laure contient et qu'elle n'a pas déplié (W brut = J/jeton × t/s ; net = brut − repos ; pas = b / t/s) :
