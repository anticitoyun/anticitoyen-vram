...
## Suite
Rejeu reporté par la Maîtresse à l'accalmie de 15 h 10 (protocole 6 fenêtres, médiane, garde load1/W). Corriger `chaine.sh` avant relance : tuer le process serveur par PID réel (pas seulement par port) dès l'abandon du curl-loop, et fiabiliser `v()` contre un fichier absent.
