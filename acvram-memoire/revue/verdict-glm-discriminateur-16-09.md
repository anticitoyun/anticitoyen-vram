...
Océane, à sec (aucun GPU, `CUDA_VISIBLE_DEVICES=""`), ordre Sage
(`revue/sage-glm-mma0-verdict-16-09.md` §3, main f48c0f7).
...
(1b) de Sage (« l'échelle élargit l'étendue intra-bloc, le noyau est
...
l'application de l'échelle sur le chemin MMA réel — Laurine, comme prévu par
Sage pour le cas (1a) : correctif sur `_forward_prefill_grouped`/MMA (piste
déjà notée dans `oceane-equivalence-pile-gateup-16-09.md` : coupure nette à
...
commit, re-PPL du même converti scellé ≤ 1,010 (réfuté > 1,015). Manon
attend (§5 de la note de Sage).
