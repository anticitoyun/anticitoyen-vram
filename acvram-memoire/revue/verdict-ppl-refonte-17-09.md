# Verdict — refonte des PPL (poste7 § 9-10) : GLM avec `[gMASK]<sop>`, PPL par fenêtre, médianes ; Coder recalculé
...
commit : arbre poste3 59cd2ca pour les prises (acvram GLM sur 59cd2ca, avant fusion de 505f31c) ; protocole `protocole-ppl-corpus-prive-17-09.md` + poste7 § 9-10
...
- « Publié tel quel dans le journal » (poste7) : complétion brute GLM effondrée depuis toujours, silencieuse — le gabarit de chat met `[gMASK]<sop>`, `encode(add_special_tokens=True)` ne le met pas (vérifié) ; correctif serveur 505f31c (poste4).
- Arbitre prefill / PPL en mode décodage / duel : à refaire avec `encode_brut` par séquence (poste7 § 10.2) — hors de ce verdict.
