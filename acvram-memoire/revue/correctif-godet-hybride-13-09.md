...
poste3. Suite à `part-reelle-mla-sous-graphes-13-09.md` (découverte du bug) et
à la demande de chef de corriger tout de suite, puisque c'est moi qui l'ai
en main.
...
dépôt : sans `sys.path.insert(0, ".../travail/poste3")` en tête, `import
acvram` retombe sur le venv partagé
(`anticitoyen-vram/.venv` → `anticitoyen-vram/acvram`, la copie **hors
worktree**), pas sur `travail/poste3/acvram`. Une première tentative de mesure
...
## La mesure qui compte (chef) : `ACVRAM_CHRONO_SYNC=1`, vrai `b_reel=12`
...
`campagne-chrono-sync-11-09.md`, `duck-poste3-12-09.md`,
...
poste4) ; la prédiction de gain du 13/09 matin (-5/-18 %) n'a pas encore été
recalée sur la mesure ×1,328 — à faire quand poste4 reprend le bead.
