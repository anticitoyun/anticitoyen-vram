# Banc énergie 4 moteurs — CHAÎNE INTERROMPUE après 3 défauts, seule A(1) valide — 22/09 (poste2)

* instrument : `scratchpad/poste4-b12-21-09/chaine-energie-4moteurs.sh` (poste4, 7a115e50/7d848001), ordre A L V T T V L A à b=12
* commit : main/poste2 à jour, 2 correctifs préalables déjà poussés par moi (56015c9a collision `ACVRAM_CARTE_TENUE`, 3387eeee `local` co-dépendant) — puis 3 défauts supplémentaires découverts, non corrigés, chaîne arrêtée
...
* **défauts nommés, non corrigés (poste4 les prend sur sa branche)** :
...
## Suite
poste4 corrige ses trois points sur sa branche (timeout 300 s + garde de nettoyage, VRAM libre vérifiée avant chaque bras, chemin yaml absolu). Je relance la chaîne complète au SHA de poste4 dès que poste3 rend la carte (b=8 + KL trtllm en cours, ≤15 min).
