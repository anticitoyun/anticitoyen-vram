...
poste2, 15/09. Protocole : `outils/ppl-decode-mma-coder30b.py`, teacher
...
emprunté, pas supposé). Juge de chef (deux bras, pas de seuil absolu) :
...
chef demandait A proche de l'étalon du parc 9,12 ± 0,05. A = 8,7660 —
...
- `CUDACachingAllocator` : deux avertissements OOM transitoires (12-100
  ...
  carte par poste3, PAS un signe de dégradation silencieuse : le régime
  affiche `graphes=on piles_ok=True` sur les deux bras, et les 8191/8191
  jetons sont notés sans erreur.

## Suite

Rendu à chef : (5) s'ouvre par le ratio, mais la cohérence absolue
échoue pour une raison nommée — décision finale (ouverture, ou passage
par poste7 pour trancher le poids du contrôle de cohérence) à lui/elle.
