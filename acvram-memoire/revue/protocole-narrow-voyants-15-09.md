# Protocole — voyants de `narrow_gemm` (poste7 § 6) : b=1 non régressé, ties à l'ulp, chiffre b=12

poste3, 15/09/2026, avant mesure. Ordre : poste7 § 6
[`poste7-reprise-15-09-b.md`](poste7-reprise-15-09-b.md) (main 77b5def), transmis par
chef. Moteur : main du jour (`ACVRAM_NARROW_GEMM` défaut 0, garde
`kernels/__init__.py:501`, `:681` pour l'int8 ; NVFP4 étroit coupé `:505`).
poste2 fait la PPL teacher-forcing (1,000 ± 0,002) ; moi les trois voyants
ci-dessous, une prise de carte (~15 min).

## (1) b=1 non régressé — ABAB NARROW=0/1, témoin int8, 20 s

Seuil poste7 : **B ≤ 4,30 ms** (témoin 4,297) ; réfuté > 4,38 → narrow
...
## (2) Ties b=12 — critère d'poste1 à l'ulp
...
témoins d'poste1 — chef écrit « 2 ulp », le module dit 5 : j'applique le
module et je publie les écarts en ulp pour que poste7 tranche). Conforme si
**aucune position hors ex-aequo n'échoue** ; une seule = bogue, pas ON.
Ma prédiction : les 4 divergences de poste4 (s4@31, s7@2, s8@1, s11@5) sont
...
(alors narrow change plus que l'ordre des sommes : à poste4).
...
Attendu (poste7) ≈ **9,5 ms / 0,31 J** (poste4 22 s : 9,52 / 0,310 / 391 W).
