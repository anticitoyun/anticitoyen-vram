# Chantier C4 — godets sur `b` : prérequis `_bind_hybrid`, preuve à sec, `ACVRAM_GODETS_B` (19/09, branche `oceane-c4-godets-b`)

Source : `sage-poursuite-chantiers-19-09` § 2 ligne C4 ; `MECANISMES.md` « Prérequis bloquant du chantier godets sur `b` ». Tout à sec, CPU, aucune carte.
...
3. Drain 12 → 1 (`mesure-debit-concurrent-13-09.py`, 12 invites, longueurs échelonnées) godets vs `ACVRAM_GODETS_B=0` : captures, replis eager, temps total ; seuil proposé ci-dessus, à faire valider par Sage avant de lancer. Nemotron-3.5-Lightning-30B-A3B-nvfp4 en second si le temps reste (Mamba2 : l'état du fantôme bouge, c'est lui que la correction de ce soir protège).
