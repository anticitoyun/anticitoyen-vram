# Sage — lancement du travail de groupe après relecture TensorRT-LLM / ggrun (14/09, ordre de l'utilisateur)

## 1. Ce que la relecture a rendu : rien de neuf chez eux, un trou chez nous

**TensorRT-LLM** `43cd45f9f` : 2 commits depuis `9cd401a` (ma veille du 14/09), tous deux infra et tests (`#18126` NGC PyTorch 26.08, `12c5de87f` borne de lot dans un test llmapi). **ggrun** `f777280` : 0 commit. `sage-veille-trtllm-ggrun-14-09.md` reste la référence ; P1-P8 et leurs seuils sont inchangés, rien n'est resserré.
...
| index : 9 703 tenseurs, 48 shards ; 59 Go sur disque | `model.safetensors.index.json` | Manon vérifie les 48 fichiers présents et leurs tailles (l'index ne dit pas ce qui est sur disque) |

## 2. Prérequis 6-pré (Océane, à sec, ce matin, avant 14 h) — scellé
...
## 4. File du jour — ce qui change par rapport à `sage-ordre-15-09` + `sage-reprise-14-09`

Carte, dans l'ordre : **0 Laurine** llama.cpp b=1 sous ncu → **2 Laurine** bt=32 → **4 Laure** modes + **P5** (20 min, garde de décodage sous prefill 512/2 048/8 192) → **5 Laurine** pas complet MoE MMA sous graphes → **P1 Laurine** (2 h) → **6 Manon** conversion GLM (soir), **seulement si 6-pré et l'équivalence sont verts**.

Sans carte, en parallèle :

| session | ordre | seuil / ce qui rend « faux » |
|---|---|---|
| **Océane** | (1) **6-pré** avant 14 h ; (2) gardes `energie.py` (`sage-ordre` (b)) ; (3) **P3** `hit_rate` + `cached_prompt_tokens` dans `/metrics` et `regime_ligne` ; (4) **P4** porte de spéculation (moyenne glissante 64 pas, coupe < 1,15, `auto` = n-gram si lot ≤ 8) | (1) §2 ; (3) un banc « tour d'agent » de Laure plus tard doit lire ≥ 85 % de réutilisation, sinon bogue ; (4) taux 1,05 simulé → coupée en ≤ 64 pas |
| **Manon** | (1) 48 shards présents, tailles vs index, `load_model_spec` sur le config : chaque champ MLA/MoE **lu de la source** (liste des ABSENT publiée) ; (2) équivalence 2 couches (§2) ; (3) corpus et VRAM de conversion prédits avant ; (4) A7 alpha commun si le temps reste ; (5) conversion le soir | (2) réfuté → pas de conversion ; (5) PPL NVFP4 ≤ étalon × 1,01, réfuté > 1,02 |
| **Laurine** | entre deux tours de carte : **P7** tampons tournants > 96 Mo dans les bancs de noyaux (tuile, NV, bt) | le classement bt=32/64 change entre L2 chaud et froid → le banc chaud mentait |
| **Katy** | audit « appareil » (N/M, fichier:ligne), envoyé, vérifié par le dépôt, **jamais attendu** | `campagne-20s-vllm-14-09.py:45` doit figurer dans la liste |
| **Jérôme** | commite ANNUAIRE + cette note ; ouvre le bead 6-pré ; correction datée B2 dans `references-moteurs-2026-09-09.md` (veille §1) ; distribue ; relevé `nvidia-smi` de début (services 8081-8083 éteints à noter) | — |
