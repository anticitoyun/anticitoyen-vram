# poste7 — reprise immédiate (utilisateur 05 h 56 : « on poursuit avec reprise tant que les tokens sont disponibles ») : la fin de carte de 08 h 00 tombe, la file continue ; ordre des chantiers par valeur mesurée, scellés déjà écrits, un membre = une file (20/09, 05 h 56, horloge machine)

Source : `poste7-bilan-nuit-20-09` § 6 ; scellés de `poste7-c1-ferme-c15-prefill` § 2 (C15-prefill), `poste7-glm-b1-noeuds-c15-niveau3` § 2 (niveau 3), `poste7-cloture-nuit-0540` addendum 05 h 45 (niveau 2 biais), `poste7-fiches-c5b-c13c-c14b` § 3 (C14-b), `poste7-c13c-forme1-faux-forme2-tf32` addendum 01 h 25 (C13-c structure), `poste7-parc-c5b-poste-e-niveau2` § 1 (gemma, Ornith), `poste7-c15-niveau3-coder-faux` (sélection ≤ 3 µs). Règles inchangées : ETAT seule entrée, pointeurs, un scellé par fenêtre, `date` avant toute heure.

## 1. Ce qui change
La clôture de 08 h 00 (bilan, dernier push) **n'a plus d'objet** : le bilan de 05 h 51 tient lieu de point d'étape, chef continue fusions et ETAT au fil ; la question 119B reste posée (§ 6 du bilan), **C9 reste en pause** jusqu'à la réponse ; l'équipe garde son rythme (une fusion par verdict, pas de « fin de nuit »). Rangs restants de `poste7-cloture-nuit-0540` (ligne de régime → 0.6.25, gemma repli annoncé, vLLM KV fp8, C3 MTP, tri des worktrees) : **ils passent en tête** de chaque file avant ce qui suit.

## 2. Files (par valeur mesurée sur le comparatif ; scellés = ceux des notes citées)
| rang | poste1 (code, à sec puis pointeur) | poste2 (carte) | gain visé (mesuré, pas promis) |
|---|---|---|---|
...
Un scellé réfuté ferme son rang et passe au suivant, sans note de poste7 sauf prédiction réfutée ou changement d'ordre ; une prédiction chiffrée avant chaque fenêtre, avec l'issue qui gênerait son auteur.

## Ordre
* **poste1** — rangs restants de la clôture (ligne de régime, gemma repli annoncé, C3 estimation) puis 1 → 2 → 3 → 4 → 5 → 6 → 7 ; sous-agents autorisés (un par chantier, worktree, chaîne carte livrée avec le scellé en tête).
* **poste2** — vLLM KV fp8 (en cours), bras éco de 0.6.25, puis les fenêtres dans l'ordre des pointeurs d'poste1 ; trous : hygiène (C5-b nsys 10 min, déterminisme ON/OFF 20 min, PPL longue 40 min).
* **chef** — tri des worktrees, 0.6.25, fusions à leurs verdicts, un .deb par changement de défaut (bras éco à chaque fois) ; ETAT et INDEX au fil ; **question 119B à l'utilisateur** (§ 6 du bilan) : C9 attend sa réponse.
