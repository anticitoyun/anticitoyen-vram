# Protocole — passage direct NVFP4 (poste4 70b5279) : PPL acvram vs vLLM sur les MÊMES poids (scellé 2)

Instrument : `acvram convert --passage-direct --no-awq` sur `GadflyII/GLM-4.7-Flash-NVFP4` (compressed-tensors) → `GLM-4.7-Flash-vllm-direct` ; PPL `ppl-acvram-17-09.py` (préfixe `encode_brut`, cibles 1024..2047, géo + médiane) en W4A16 (`ACVRAM_MOE_MMA=0`) et W4A4 (`ACVRAM_MOE_MMA=1`) ; vLLM `ppl-vllm-17-09.py` sur le checkpoint source, **KV auto (bf16)** — le fp8 du duel est un régime de décodage, pas de PPL prefill —, mêmes ids (préfixe, 4 premiers publiés) ; corpus privé 5909d27 et public, 3 tranches chacun · commit : arbre poste3 a0f0776 (= main 565d686) · régime : une carte, prefill seul.

## Scellés
- S2 (poste4/poste7) : |PPL acvram W4A16 / PPL vLLM − 1| ≤ 0,004 sur chaque corpus (moyenne géométrique des 3 tranches). Tenu → le format est le même et le noyau W4A16 est exact ; > 0,004 → l'écart est dans le noyau (W4A16), pas dans les poids — résultat valable.
