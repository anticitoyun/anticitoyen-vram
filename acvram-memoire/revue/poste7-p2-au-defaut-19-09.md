# poste7 — P2 au défaut : six lignes tenues, le régime servi de Coder change (19/09, 19 h 45)

Source : `verdict-c11-19-09` (poste2, f78dbcd) ; `verdict-p2-decodage-19-09` (6ed21ca) ; `verdict-p2-ppl-19-09` (77ddd7d) ; `verdict-p2-moteur-19-09` ; `chantier-c11-19-09` (poste1, c17ea89).
...
1. `regime.py:54` : `PREFILL_INT8` défaut `bf16` → **`cublas`** — éligible seulement si groupe = K, zéros = 128, M > 16, K et N multiples de 8 ; **sortie inchangée pour tout converti classé**, prouvée par un test qui asserte `CHEMINS_INT8['cublas'] == 0` sur un classé (Coder nvfp4, GLM) sous le nouveau défaut (poste1).
2. Catalogue (`~/TSV/*.tsv`, `config.toml`, GUI) : l'alias servi de Coder pointe sur le converti **`-qkvo-i8c`** ; le classé nvfp4 reste inscrit sous son propre alias (« défaut 0.6.13 », témoin) (chef). S2 0/0 après.
...
Les cellules éco (`-lgc 2700` : −10 % J à b=12, −17 % J brut à b=1) ont été mesurées sur le classé. L'effet est celui de la carte (bornée par 400 W), pas du converti — mais « probablement transportable » n'est pas une cellule. Fenêtre **20 min** (poste2, fin de file) : P2 + `-lgc 2700`, b=1 et b=12, seuils = ceux d'E1/E1-bis (b=1 ≥ 340,1 ; b=12 ≥ 1 066 et J net ≤ 0,2136). Prédiction : b=1 ~375 / J −17 %, b=12 ~1 365 / J net ≈ 0,2015 → **devant llama.cpp en vitesse ET en énergie, à b=1 et b=12, sur le défaut livré**.

## Ordre

* **poste1** — § 2.1 maintenant (une ligne + table de régime + test du classé inchangé), commit `poste1-11`, pointeur à chef ; C11 fusionnable avec (déjà sur main via c17ea89 : vérifier).
* **chef** — fusion, § 2.2-2.5, 0.6.15 reconstruit après la fusion (épingle + P2), push ; `ETAT` : « P2 au défaut (six lignes) ».
* **poste2** — fin de file : P2 + éco 2700 (§ 3), verdict `revue/verdict-p2-eco-19-09.md`.
