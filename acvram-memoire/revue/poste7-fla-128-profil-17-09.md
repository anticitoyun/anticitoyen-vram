# poste7 — fla à 128 t/s : le scellé est réfuté tel qu'écrit, et le profil est la suite qu'il prescrivait — les deux à la fois, pas l'un contre l'autre (17/09)

Entrée : chef — JIT fla sm_120 OK ; Qwen3.8-27B b=12 **128 t/s** (torch 97 ; scellé ≥ 400, faux si < 250). Régression 0c91092 (réserve de préfill lisait `kv_max_tokens` au lieu de `max_model_len`) : notée, cause probable des faux « dégradé » du palier 2, à rejouer seulement si une cellule en dépend.
...
* colle ≥ 50 % ⇒ le remède est **l'état par lot** (un tenseur [b, …] par couche, mis à jour en place, capturé dans le graphe ; zéro stack/clone par pas) — poste4, à sec, et le ≥ 400 reste l'objectif, rescellé après le profil sur les ms mesurées.
...
* GEMV ≥ 40 % ⇒ le goulot est le dense 27B lui-même, et 400 t/s n'était pas atteignable (12 × 27B : plancher 8,4 ms ⇒ ≤ 1 400 t/s théoriques, donc si, mais pas avec cette GEMV) — on revient à la GEMV ≥ 85 % de `poste7-lecture-profils`.
...
Fenêtre : ≤ 30 min (poste3). Aucun noyau, aucune correction avant le JSON.
