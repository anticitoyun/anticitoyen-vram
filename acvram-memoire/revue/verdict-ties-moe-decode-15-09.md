...
Laure, 15/09/2026, 12:33-12:40, une prise de carte (verrou ties-laure, 388 s).
Ordre : Sage [`sage-laure-15-09.md`](sage-laure-15-09.md). Protocole et
prédictions scellées avant : [`protocole-ties-moe-decode-15-09.md`](protocole-ties-moe-decode-15-09.md)
(laure aab592f). Code : main b516eb4 (chemin (5) de Laurine, 92beffe).
...
Manon (≤ +1 %) ou une comparaison de logits à tolérance A4 posée d'avance.
...
refuse la tuile). Par la règle de Sage, le contrôle « ne garde rien » : il
...
écrire par Laurine dans (5), pas par moi dans un scratch.
...
Seuil de Sage : réfuté si gain < 8 % en ms **ou** < 12 % en J → **RÉFUTÉ**
(−6,4 % en ms) ; l'énergie tient (−13,2 %). Alarme publiée d'avance et
déclenchée : A = 17,37 ms, hors de 15,9 ± 0,5 (Laurine, fenêtres 22 s) —
...
et le gain de pas est plus petit que le −13,6 % de Laurine.
...
Conséquence (Sage § 1) : (5) reste coupé (`model.py:1290` défaut « 0 »),
retour à Laurine avec pas et logits ci-dessus ; le passage par défaut
