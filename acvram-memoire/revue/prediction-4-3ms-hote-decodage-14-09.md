...
Océane, 14/09/2026, AVANT profilage. Laurine a mesuré (noyaux) : pas total
16,5 ms, GPU (rejeu du graphe) 12,25 ms, donc **4,3 ms hors rejeu**, côté
hôte. vLLM fait le pas ENTIER en 10 ms. Jérôme : pas ≤ 13,5 ms (host ≤
...
## Profilage (outils/profil_cpu_pas.py, branche Laurine adfad5b) — hypothèse confirmée
...
Laurine — seule la RÉPARTITION relative (où va le temps) est fiable ici.

## Ce que la correction demande, et pourquoi je m'arrête avant de l'écrire

La suite (Jérôme + Laurine) : garder le jeton échantillonné comme
...
dont dépend tout le reste du chantier (Laurine, Laure, Manon) — je
...
### Ma première explication était fausse — corrigée par Jérôme
...
hôte hors-attente (~6,4 % des 4,3 ms) était masquable. Jérôme a
...
**Contrôle (1) de Jérôme, fait par événements CUDA directs** (pas
