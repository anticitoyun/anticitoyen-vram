...
commit : arbre laure fc04517 (= main, défaut `ACVRAM_PREFILL=bf16`, 0971c90) ; régime lu : `regime_ligne()` → `ACVRAM_PREFILL` = `w8a8` / `bf16` selon le bras, tout le reste au défaut, MOE sans objet (dense), une carte
scellé (Sage, amendement) : PPL `bf16` meilleure de 2-5 % ; j/s `bf16` −25 à −45 % ; chantier « GEMM W4A16 à déquantification fusionnée » si > 25 % de j/s perdus ET ≥ 1 % de PPL gagné
...
- Vitesse dense : σ 15 et 1 j/s sur 7 répétitions ; l'écart 56 % est vingt fois hors bruit. Régime : GEMM bf16 après déquant contre `_scaled_mm` FP8 avec requantification des poids par ligne à chaque appel (Laurine 429902c) — le coût de requantification à chaque prefill est la cause probable de la lenteur du w8a8.
