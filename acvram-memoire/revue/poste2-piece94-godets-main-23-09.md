# Pièce 94 — capture-godets rejouée sur main HEAD, 4 alias (ordre chef)
...
commit : main `ff264256` (fusion locale de `poste2` sur `origin/main`, worktree `poste2-w-21-09`, `git worktree list` confirme l'arbre)
...
mesuré : `acvram.__file__` relevé en tête de chaque prise = l'arbre `poste2-w-21-09` (jamais l'installation) aux 4 prises
...
suite : chef — les 8 verdicts du 19/09 peuvent être requalifiés « tenus sur main » ; ma file : cellule (a) attention 4 vs 8 warps, puis (b) rejeu vLLM b=12 (89) aux nouveaux défauts.
