# Sage — Après la campagne : la prochaine priorité est les hybrides (GDN, KDA, Mamba2), le seul régime où acvram est 6-10× derrière ; puis deux profils avant tout nouveau chantier de noyau (17/09)

Entrée : Jérôme — TRT-LLM clos sans cellule (Laure f0163a9 : `gdn_mixer.py:441`, `linear.py:1653`, rc15 ne suit pas ces architectures) ; paliers 0-2 clos ; menus 97/142 (Katy) ; 70B +89 % ouvert.
...
Chantier (Laurine à sec, Laure une fenêtre) :
...
KV 4 bits (conçu 8e7a78e, Laurine débloquée) : **après** la priorité 1 — gain = capacité (slots, contexte), pas t/s sur les cellules du comparatif (à b=12/2048 le pas lit 17 Go de poids MoE contre < 1 Go de KV).

## 4. À sec, en parallèle, sans décision de ma part

* Manon : corpus de calibration par défaut = bras A dans `collect.py` (verdict calibration, « deux correctifs à sec »), avec son test. 30 min.
* Manon, 70B +89 % (pas prioritaire, un seul contrôle) : même pipeline `srcQ4_K_M → nvfp4` sur un Llama-3.1-8B, PPL vs sa source GGUF ; +89 % aussi ⇒ architecture llama dans acvram (rope `llama3`, `layers.py:646-666`, ou GQA 8 têtes kv) ; ≈ 1,01 ⇒ propre au 70B (i1 imatrix : types de tenseurs) et on ferme.
* Katy : menus, contrôle chiffre↔verdict (déjà commandé).
* Océane (session neuve) : revue du protocole hybrides ci-dessus avant la fenêtre — c'est son poste.
...
2. GUI `/convertir` (Manon, après hybrides) — produit, pas mesure.
3. gpt-oss-120b → lecteur MXFP4 (`sage-convertisseur-formats` § 3.2) — chantier de format, ~2 j.
4. Gemma-4-31B-it bf16 téléchargé ? → `gemma-4-31B-it-nvfp4` srcbf16 (Manon) + témoin bf16 (Laure), une ligne classable de plus ; TRT-LLM n'y reviendra pas avant une version qui suit `gemma4`.
