...
Laure, 16/09/2026, avant mesure. Ordre : Jérôme (ETAT.md l. 2), scellés Sage
`sage-glm-pile-correctif-16-09` § 1.5 (iii) et § 6.3. Arbre : travail/laure
figé au commit de ce protocole (code `acvram/` = main **e84a43d**, correctif
Laurine 01c48ef fusionné, t-qa 210/210). Extension compilée hors verrou
(ninja, 85 s, cache `kernels-84a817c8c4ce`, empreinte du source contrôlée au
chargement `kernels/__init__.py:376-386`).

## Montage
`acvram eval` (ligne de Manon, `verdict-glm-mma0-confondant-16-09`) : corpus
...
de Sage). Compteurs B : mis à zéro **0**, saturés **0** ; si mis à zéro > 0 il
est < 0,01 % des blocs (queue basse au-delà des 16 jetons d'Océane) et c'est
