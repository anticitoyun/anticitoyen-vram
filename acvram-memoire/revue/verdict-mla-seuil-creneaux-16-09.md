...
- **commit** : main **dcb09ba** (travail/laure f7d8afb) ; régime W4A16 prefill et décodage, `-k48`, `ACVRAM_HYBRID_SLOTS = b` à chaque fois (règle § 10)
- **régime** : graphes actifs (ties) ; contrôle **eager** (`TIES_EAGER=1`) à b=4/SLOTS=4 et b=1/SLOTS=1
- **scellé** (Sage § 10) : faux dès 5 → `static_load`/magasin au-delà de 4 ; juste à 5 et 8, faux à 12 → godet 16 / `_SID_REMBOURRAGE`. (2) b=1 et b=4 du duel publiés seulement s'ils passent
...
- **verdict** : **aucun des deux motifs de Sage** — c'est faux **dès b=1**, pour tout nombre de créneaux, **seulement sous graphes** ; le chemin à créneaux est juste en eager. (2) **b=1 et b=4 du duel ne passent pas** (5/7, 19/28) : non publiés. Le duel entier (b=1, 4, 12 : `certifie` à SLOTS=b, graphes) tournait dans ce régime

## 1. Le motif temporel, pour Laurine (fichier:ligne à confirmer, pas une piste vague)
...
## 2. Conséquences
- **Duel prise A** : les trois lots (b=1, 4, 12) sont à refaire après correctif — ou, en attendant, mesurables **en eager à créneaux** si Sage veut un chiffre juste tout de suite (plus lent, mais vrai).
