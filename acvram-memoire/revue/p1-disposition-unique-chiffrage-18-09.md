# P1 — disposition unique : chiffrage à sec des deux formes avant d'écrire (Laurine, 18/09, ½ j ; sage-p1-disposition-unique-18-09, ordre corrigé de Jérôme)
...
(½ o de codes + ⅛ o d'échelles) × 48 = **14,5 Gio** — Laure l'a mesuré
...
`banc-marlin-decode-18-09.py` + bras (b). Porte à sec (Sage) : (b) ≥
...
## 6. Écrit (GO Sage 18/09) — et une correction du § 3 avant la carte
...
Livré (branche laurine) : `nvfp4_gemv_marlin[_gateup]` (acvram_kernels.cu :
...
Ordre Laure : `CUDA_VISIBLE_DEVICES="" python -c "from acvram.kernels import
...
## 7. Disposition unique dans le CHARGEUR (Laure : 17,04 Gio réservés, 21/48 couches exilées)
...
## 8. Biais du GEMV (b) in situ (+0,008 PPL) — instruments et hypothèses (sage-p1-situ-verdict-18-09)
...
(v1, (b), témoin, chacun contre fp32) : `biais_signe = moy(Δ)/moy|y|` (Sage,
...
### GLM prefill Marlin −0,003 et GEMV (b) décodage +0,008 : même famille, à nommer ensemble (Sage)
...
### Verdict de Laure (revue/verdict-biais-gemv-b-18-09, 9584ad7) et suite
...
(b)** — ma prédiction (« suit le préfixe ») est RÉFUTÉE, celle de Sage tenue.
...
Instrument livré pour la piste de Sage (premier pas divergent) :
