...
- **scellé** (Sage) : reproduire, corriger, test qui casse si la faute revient ; inventaire des tests
...
- **verdict** : cause nommée, indépendante du passage direct (tout converti à piles y passait ;
  ...
  pareil). Correctif (laurine) : `dequantize_nvfp4` voit les échelles uint8 comme E4M3 ; la pile
...
- **inventaire** (`git log -S passage_direct -- tests`, `-S DISABLE_KERNELS -- tests`) : aucun test
  ...
  (70b5279, 8164622) et ses tests ne coupent pas les noyaux. Prédiction de Sage tenue.
- **reste** : rejouer la PPL sous `ACVRAM_DISABLE_KERNELS=1` sur carte (Laure) : attendue égale à
