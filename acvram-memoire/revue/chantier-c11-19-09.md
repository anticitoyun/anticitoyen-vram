...
Objectif (Sage, `sage-p2-dec-c11-c6-19-09`) : P2-déc b=12 FAUX (1 062,6 t/s < 1 334, J × 1,30) parce qu'à 2 ≤ M ≤ 16 un poids i8c (groupe = K = 2 048) n'avait que le repli déquant : `gemm_etroit.eligible` refuse G > 128 (tuile K = groupe, 393 Kio de shared à G = K), `torch._int_mm` exige M > 16, `narrow_gemm` CUDA non pris.

Prédiction scellée (Sage, recopiée) : b=12 i8c **≥ 1 334 t/s ET J ≤ 1,02 × A** (A = classé au défaut) → tenu → P2 au défaut ; prédiction : tenu, même coût que le g128 du classé.

## Le geste (le plus court des deux de Sage : ni noyau, ni ptxas)
