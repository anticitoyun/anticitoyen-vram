...
## 3. Scellé, écrit avant (Laure, 15 min de carte, même instrument ncu + banc + in situ)
...
* Tenu ⇒ in situ ABAB Coder b=12, et le **scellé du chantier, inchangé : ≥ 1 300 t/s nu** (prédiction 9,51 − 1,1 = 8,4 ms ⇒ ~1 430), J/jeton ≤ 0,344 en second juge, `ppl-decode-kv` identique. Puis fermeture aussi, cellule Katy mise à jour.

## 4. Instrument (Jérôme, REGLES § 6, une ligne datée)

« `sudo -n ncu` remet l'environnement à zéro : le régime se pose dans le wrapper de la cible et se prouve par la ligne `[régime]` de sa sortie, jamais par l'environnement du shell appelant » (Laure, 18/09, deux essais à 0 noyau profilé). Parseur en locale fr et `dram__bytes_read` n/a : dans le script (Laurine), pas dans REGLES.

## Ordre

* Laurine : § 2 à sec + script ncu corrigé (3 défauts de Laure) → `verdict: revue/<fichier> — bit-exact n/20, registres, commit`.
* Laure : § 3 dans une fenêtre, ncu puis banc puis in situ → `verdict: revue/verdict-gemv-experts-x-registres-18-09.md — ms/pas ncu, tenu/faux ; t/s nu, J`.
* Jérôme : § 4 ; ETAT : « GEMV experts : dernier geste (x en registres), porte ncu ≤ 6,2 ms/pas, fermeture après quel que soit le verdict ».
