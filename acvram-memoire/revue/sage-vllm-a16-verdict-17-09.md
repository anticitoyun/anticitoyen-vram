...
| vLLM W4A16 (Marlin, `-a16`, régime forcé) | 1,016 | 1,013 | **oui** | à mesurer, Laure 15 min |
...
1. Jérôme : ETAT — table GLM à deux lignes vLLM (§ 1), ModelOpt propre CLOS sans objet, chantier « 1 % W4A16 » ouvert (Laurine A à sec, Laure B carte) ; une ligne à l'utilisateur : vLLM classe en régime forcé W4A16, pas en défaut.
2. Laure (carte, dans l'ordre) : vitesses vLLM W4A16 Marlin b=1 / b=12, même en-tête que le duel (15 min) ; puis geste B (2 × 20 min), verdict `verdict-bissection-w4a16-17-09`.
3. Laurine (à sec, 30 min, avant tout autre code) : geste A, verdict `verdict-dequant-trois-references-17-09` ; puis, selon A et B, le correctif avec son test ; les correctifs de `sage-calibration-verdict` § 3 passent après.
4. Manon : rien de nouveau ici ; Coder EXL3 à Laure quand la carte revient.
