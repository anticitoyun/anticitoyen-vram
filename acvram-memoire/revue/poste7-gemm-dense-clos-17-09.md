# poste7 — Chantier GEMM dense petit M : CLOS après trois tours (349 → 392 → 412 t/s) ; réduction fusionnée en défaut par le J/jeton ; CUDA chiffré = parité vLLM, pas plus, donc derrière la GEMV experts (17/09)

Entrée : poste3 43d025b — nu 441 t/s (ma bande « ≥ 455 / < 435 » laissait 435-455 orphelin : **quatrième fois aujourd'hui, une ligne après avoir écrit la règle** — carnet, et cette fois la règle est mécanique : je n'écris plus « faux si < Y » avec Y ≠ X) ; rondes **412 t/s, 0,967 J** (≤ 0,97 tenu) ; `_reduire_kernel` disparu, `_dense_etroit` 18,2 → 19,8 ms (l'épilogue se paie dedans), net −1,7 ms ; GEMM dense = 75 % du pas à **1,03 To/s effectifs** (58 % de la bande ; plancher 11 ms).
...
Pas actuel 29 ms dont GEMM 22 ms à 1,03 To/s. Un noyau CUDA façon Marlin à 1,45 To/s : 20 Go → 13,8 ms, pas ≈ 21 ms ⇒ **≈ 570 t/s : la parité avec vLLM (19 ms), pas l'avance.** Pour 3-4 jours de poste4 sur un modèle non classé. Il passe derrière la GEMV experts ≥ 85 % (Coder b=12, classée, cellule phare) et derrière la porte migration d'échelles (préfill) ; il ne revient que si l'utilisateur demande la parité vLLM sur les denses à b > 1 comme objectif propre — une ligne à lui porter.

## Ordre

poste4 → GEMV experts ≥ 85 % (`poste7-lecture-profils` § 2 : micro-banc octets/temps à sec, puis poste3 20 min ; scellé b=12 Coder ≥ 1 300 t/s nu, deux bandes : ≥ 1 300 tenu, < 1 300 faux ; J/jeton en second juge). poste2 → calibration Nemotron bras A (en cours). poste8 → cellules Qwen3.8 et Nemotron aux menus.
