# ABBA sampler b=12 Coder : A(lent)/B(graphe) — INTERMÉDIAIRE, B gagne +1,24 % — 22/09 (Manon)

* instrument : `scratchpad/laurine-b12-21-09/chaine-sampler-abba.sh` corrigé (A=défaut, B=`ACVRAM_SAMPLER_GRAPHE=1`, remplace l'ancien `ACVRAM_SAMPLER_LENT` qui comparait par erreur lent contre lent) ; worktree `manon-w-21-09`, répertoire `sorties-abba/` absent au 1er essai du soir (aucun rapport avec un permission menu `rm`, corrigé par `mkdir -p`)
* commit : 3a925976
* régime : Qwen3-Coder-30B-A3B-nvfp4-qkvo-i8c, b=12, `--max-model-len 2304`, ordre A0/B0 (blanc) puis A1 B1 B2 A2 A3 B3 B4 A4
* scellé (Océane) : ABBA t/s B/A ≥ 1,000 (le levier ne doit pas coûter) ; rejet si |horloge_medA−horloge_medB|/horloge_medA > 3 %
