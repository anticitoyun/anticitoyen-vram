# P3 (4) duel Qwen3-VL-30B reconverti (correctif experts groupés, carte visible) — 21/09 (Manon)

* instrument : `scratchpad/mm-qvl-20-09/chaine-p3-4.sh` (worktree manon-w-21-09)
...
* scellé : seuils H1 (Océane, ordre du jour) TTFT ≤ 0,3 s, J ≤ 60 — annoncé d'avance : « le scellé 0,094 s peut rester réfuté par la tour eager transformers, à dire » ; seuil historique § P3(4) du 20/09 (TTFT ≤ 0,094 s, J ≤ 40,0) gardé en référence
...
## Suite
Cause de l'échec de calibration (`setStorage: ... storage size of 8 are out of bounds for storage of size 0`) hors mon domaine (collecte de stats, `acvram/quant/collect.py`) — à nommer par Océane avant de juger la qualité P3 (3) qui suit. Enchaîne sur P3 (3).
