# Devstral : scaling « llama 4 » porté sur q (Laurine, 18/09, à sec, ~2 h)

Ordre Sage (relais Jérôme) : appliquer 1 + β·ln(1 + ⌊pos/8192⌋) sur q dans
...
refus de 67aa280 (Manon) tombe quand le scaling est porté.
...
- Branche de Manon (67aa280 : table Mistral3 → llama, rope_parameters → rope_scaling pour
  ministral3, import fp8) fusionnée dans laurine pour bâtir dessus.

## Preuve (à sec, CPU, fp32, hors pytest : le verrou carte de Laure refuse la suite)
...
max_model_len 16 384 se construit, régime « (servi) » ; les 4 tests de refus de Manon
...
décodage) mais pas mesuré ; la vraie conversion Devstral (Manon) et sa PPL (Laure)
suivent l'ordre Sage.
