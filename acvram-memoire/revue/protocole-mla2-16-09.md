...
poste3, 16/09/2026, avant mesure. Ordre : chef (+ demande de poste4 pour le bras narrow). Arbre mesuré :
**travail/poste3-qa @ bb9fdc4** (poste4, main fusionné), témoin `ACVRAM_MLA_UNE_PASSE=0` (ancien code, même arbre).
...
## Scellés
- poste7/chef : poste **mla_* ≤ 3 ms** (réfuté > 6) ; équivalence **top-1 identique ET cos ≥ 0,9999** contre
  UNE_PASSE=0 (pas bit-identique attendu : ordre des sommes fp32) ; **PPL B/A dans 1 ± 0,004** (3 tranches).
- poste4 : mla_* 6,4 → 1,5-2,5 ms ; pas 21,4 → 17-19 ms.
- Moi : mla_* **2,0-3,5 ms** (scores + reduce fusionnés lisent le latent une fois : 6,4 → moitié ou mieux ; réfuté
  ...
  d'poste1 : quelques refus (≤ 10) attendus par l'ordre des sommes ; PPL B/A = 1,000 ± 0,001 par tranche.
...
Durée ≈ 2 + 9 + 3 + 15 min, unité `mla2-poste3`.
