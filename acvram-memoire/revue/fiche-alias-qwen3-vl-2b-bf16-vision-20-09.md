# Fiche d'alias — Qwen3-VL-2B-Instruct-bf16-vision (20/09, 21 h 15, chef ; ordre poste7 `poste7-p3-1-clause4-int8-20-09`)
...
| **kv=int8 vs bf16 (vision, 20 images)** | **+2,02 % ± 1,5 % (SE), non résolu, sous réserve** — int8 conservé ; réfutation n = 60 après le feu vert (géo − 2 SE > +1 % → `kv=bf16(vision)` en 0.6.35) | poste7 21 h 14 |
...
Défaut nommé et corrigé le 20/09 : le bloc bidirectionnel de Gemma était appliqué à tout modèle porteur d'images (model.py:593) ; Qwen3-VL est causal chez HF (`create_causal_mask`) — porte de famille par `architectures` du manifeste (`poste1-masque-famille` 8ce208eb), refus nommé `MasqueImageInconnu` hors Gemma3/4 et Qwen2/2.5/3-VL.
