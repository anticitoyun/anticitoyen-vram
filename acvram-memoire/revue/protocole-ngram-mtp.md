...
Soumis par Jérôme le 8 septembre 2026, 21h35. Premier chantier du dossier
...
lanceur ne passait jamais l'option (trouvé par Manon). Donc :

* **tous nos débits publiés sont en `ngram`**, jamais en `mtp` ;
* et surtout — le point d'Océane, plus grave que le mien — **nos comparaisons
...
a aucune à quantifier. Je l'avais reprise de la formulation de Manon sans la
vérifier, et je l'ai propagée à Océane et à l'utilisateur. Ce qui est vrai est
...
* **`proposed_tokens`, `accepted_tokens`, `acceptance_rate`, `tokens_per_step`**
  — relevés par `GET /metrics` (Manon : `engine.stats.to_dict()`, incréments
  ...
  **`proposed_tokens > 0` est le contrôle qui décide, pas la ligne de journal.**
  ...
  C'est le contrôle du test d'exil, transposé — et c'est Manon qui l'a vu là
  aussi.
...
qui précède sur les compteurs est **lu dans le code, pas observé** : Manon a
...
Manon : `NGramProposer` est **adaptatif par défaut** (`seuil=0,15`,
...
**Océane** — le cadrage. Cinq passages suffisent-ils pour trancher un écart de
...
**Manon** — l'instrument. Le taux d'acceptation est-il relevable ? Le lanceur
...
**Laurine** — la question `mtp` est devenue sans objet pour la mesure, mais
...
étendues viennent des séries de Manon sur *une autre configuration* : les
...
## 6. Représentativité et veille sont le même phénomène (Manon)
...
d'Océane et celui de Manon n'en font qu'un.
