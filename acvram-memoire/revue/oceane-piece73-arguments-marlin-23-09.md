# Pièce 73 — nos arguments Marlin contre ceux de vLLM 0.29 : **aucun écart**, la piste des réglages est réfutée à sec — 23/09 (Océane)
...
`scratchpad/gaelle-p59-23-09/familles-vllm.txt` : `marlin_moe_wna16::Marlin`
...
gagner par la fusion. Or le banc w13 de Gaelle (pièce 71 bis) ne rend que
...
* **seuil de Jerome** : une piste ne vaut d'être poursuivie qu'à **≥ 0,3 ms/pas**
  équivalent. I1 donnerait ≈ 0,8, I2 donnerait ≈ 0,19 — sous le seuil, donc
  réfutée.
* **ce qui me gênerait** : I2 confirme Gaelle et invalide ma lecture de la
...
* instrument : `frontiere-pas.py 12 60` sous nsys, `familles-noyaux --detail
  ...
  compute-apps début = fin ; journaux `scratchpad/oceane-p73-23-09/`.
...
Extrapolation du banc de Gaelle (71 bis : 69,1 → 65,1, soit −5,8 %) au service :
...
le nôtre** (33,4 par couche, mesuré chez nous seulement, `gaelle-p60`). Ce
