# GEMM W4A16 dense à petit M — noyau, juge et micro-banc livrés à sec (Laurine, 17/09) ; la porte est le banc de Laure

Commande : sage-hybrides-etape1-close-gemm-dense-17-09 § 2. Le poste : à
...
plusieurs tranches K forcées ; **bras cassant de Sage** : échelle de bloc
décalée d'un rang → rouge. 13 tests, interpréteur Triton en fp16 sans carte.

## Porte : `outils/banc-gemm-dense-etroit-17-09.py` (Laure, ~5 min de carte)
...
OUVRE, < 0,9 FAUX (noyau CUDA, décision séparée), entre : à Sage.
...
## Palier 2 (sage-gemm-dense-porte-fermee-palier-17-09 § 3) — à sec, second commit

Banc de Laure (21033b7, M = 12) : q 0,72 · kv 0,47 · o 0,86 · gdn_qkv 1,01 ·
...
pas sans requantifier ; c'est un choix du convertisseur (Manon), pas du
...
Prédiction scellée (banc Laure, M = 12) : qkv_multi 0,95-1,05 (le gdn_qkv
...
décodage par table dans la boucle K) et c'est le noyau CUDA (§ 4 de Sage).
En situ ensuite (Laure) : pas GEMM ≈ 16-19 ms ⇒ 500-600 t/s.

## Défaut, tête et témoin (troisième commit) — sage-gemm-dense-palier2-non-ouvert-17-09

- **Défaut `ACVRAM_DENSE_NVFP4=triton`, bascule `ACVRAM_DENSE_NVFP4_MIN_M=4`**
  (Laure 0690bd4 : b=12 128 → 349 t/s, J/j ÷ 2,7, b=1 et ppl-decode-kv
...
- Scellé de Sage pour la tête (Laure, après Nemotron) : b=12 349 → ≥ 400
...
## Dernier tour : réduction fusionnée (épilogue « dernier bloc ») — sage-lm-head-392-verdict-17-09
...
Scellé de Sage (Laure) : ≥ 455 t/s NU tenu / < 435 faux, J/jeton ≤ 0,95× ;
