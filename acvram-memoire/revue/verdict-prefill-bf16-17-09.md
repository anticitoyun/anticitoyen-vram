...
commit : arbre laure 38e3416 (= main a83cfee, diff de Laurine 429902c inclus)
régime : identique aux prises de référence (`passage-direct-17-09`, 1,0280) et tout-torch (`bissection-suite`, 1,0104) ; cause nommée par Laurine à sec : expert partagé + couche dense 0 en W8A8 caché (`nvfp4_mm_w4a8`, poids requantifiés E4M3 par ligne + activations FP8, `_scaled_mm`), erreur RMS 3,6-4,0 % contre 0,14 % en bf16
scellé (Laurine/Sage) : ≤ 1,014 → seul coupable · 1,014-1,024 → une part · ≥ 1,024 → piste réfutée
...
- Tous les convertis maison (`-k48`, A, B, Hadamard, sansawq) ont été jugés sous `ACVRAM_PREFILL=a8` : leurs PPL prefill portent le même 1,8 % ; à refaire sous le nouveau défaut dès que Laurine le pose (ou en `ACVRAM_PREFILL=bf16` explicite) — 10 mesures × 1 min, une passe.
