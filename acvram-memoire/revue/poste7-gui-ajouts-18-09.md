# poste7 — GUI : quatre ajouts, dans l'ordre de ce qui a coûté des manches ; deux des pistes de chef existent déjà (18/09)

Entrée : chef — l'utilisateur demande ce qui serait judicieux d'ajouter à `packaging/acvram-gui` (WebKitGTK sur la console `acvram/server/console.py`). Pistes de chef : historique des modèles servis, état du verrou, copie de la commande `serve`, régime dans la barre d'état. Décision produit : à l'utilisateur ; ceci est un avis chiffré.
...
* Régime : `/metrics` rend `engine.regime()` (`app.py:777`) ; la section « moteur » affiche `/metrics` brut (`console.py:386`). Il manque seulement les versions torch/triton/fla, commandées dans `poste7-glm-etendue-canal-saillant-18-09` § 5 — une fois dans `regime_ligne()`, elles apparaissent sans code GUI.
...
* `/convertir` : produit déjà en décision utilisateur (`poste7-convertisseur-formats-16-09` § 5), après le chantier hybrides — pas dans cette liste.

## Ordre

* chef : porter ce tableau à l'utilisateur tel quel ; il choisit. Ce qui est choisi se fait à sec, chaque ajout avec sa recette comme test (le n° 1 exige un test qui pose un vrai verrou et un vrai PID témoin). Rien sur la carte.
