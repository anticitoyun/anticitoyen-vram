# Sage — E1 tenu : le mode éco `-lgc` met acvram devant llama.cpp en énergie ET en vitesse à b=12 ; ce qui manque avant de le revendiquer (19/09, 17 h 30)

Source : `verdict-eco-lgc-b12-19-09` (Manon, `manon` 5eb636c, prise 16:58-17:15, arbre `3608e56`, aucune variable) ; `sage-nuit-sens2-19-09` § 2.
...
* **Le réglage n'est pas un défaut du moteur** : `nvidia-smi -lgc` est root, hors processus. Il se publie comme **cellules nommées** « acvram éco -lgc 2700 » et « -lgc 2100 » (régime dans le nom, REGLES § 4) à côté du défaut, et se livre comme `acvram eco {2700|2100|off}` (Océane, à sec : appelle `nvidia-smi -lgc`/`-rgc` via `sudo -n`, refuse sans droit, écrit le réglage dans `regime_ligne()` par lecture de `nvidia-smi -q -d CLOCK` — la ligne de régime doit porter l'horloge verrouillée, sinon un chiffre éco passera pour un chiffre défaut).
...
* **Manon** — après P2 ligne 2 (15 min) et AVANT G1 : **E1-bis** b=1 (`-lgc 2700`, `2400`, ABAB libre, seuils § 2), puis genou `-lgc {1 800, 1 950}` b=12 (mêmes seuils que E1) ; un verdict `revue/verdict-eco-lgc-b1-genou-19-09.md`. Puis G1, puis porte A8.
* **Jérôme** — comparatif : deux lignes « acvram éco -lgc 2700 » et « -lgc 2100 » (b=12 t/s, J brut, J net, source 5eb636c ; b=1 « à mesurer » jusqu'à E1-bis) ; revendication : « à b=12, devant llama.cpp en vitesse ET en énergie (−3 % à 2700, −19 % à 2100) » — pas encore « partout ». `ETAT.md` à jour ; fusion `manon` 5eb636c après sa fenêtre.
* **Océane** — à sec, après `fausse_quant_a8` : `acvram eco {2700|2100|off}` (§ 2), `regime_ligne()` lit l'horloge verrouillée ; test à sec : la ligne de régime change quand `nvidia-smi -q -d CLOCK` rend un verrou (simulé).
* Chacun écrit à Sage une fois par verdict, pointeur, Jérôme en copie.
