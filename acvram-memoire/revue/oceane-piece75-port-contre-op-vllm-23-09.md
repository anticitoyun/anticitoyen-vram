# Pièce 75 — notre port Marlin MoE contre l'op `fused_marlin_moe` de vLLM, au même banc, sur les mêmes octets — 23/09 (Océane)
...
Les chiffres de banc existants ne tranchent pas : ils viennent de deux bancs différents (Gaelle 61/0 : op vLLM
...
1. `preparer` (venv vLLM) : une pile de 128 experts aux formes du Coder (K 2 048, N 768, gate‖up 1 536, down
   ...
   disque avec x, les 20 routages réels de la cellule b = 12 (`gaelle-p62-23-09/routages-cellule-b12-20.pt`,
...
* **alarme 2** : si V à L2 froide s'écarte de plus de 15 % des 57-67 µs/couche de la 61/0, le harnais n'est pas
  le même régime que celui de Gaelle, et je le nomme avant de conclure.

## Verdict — 23/09 08 h 52 (Océane)

* **instrument** : `outils/gpu/mesure/banc-marlin-moe-p75.py`, chaîne `scratchpad/oceane-p75-23-09/chaine.sh` ; résultats `resultat-vllm.json`, `resultat-acvram.json`
