# Verdict — trois déquantifications d'un expert NVFP4 (ordre Sage, 17/09) : vLLM = nvfp4.py au bit ; noyau à mesurer sur carte ; le candidat « ordre bloc × global » ne peut pas produire d'écart
...
- **scellé** : max |Δ| ≤ 1 ulp bf16 entre les trois. Prédiction de Sage : vLLM et nvfp4.py s'écartent
...
- **mesuré** (3 experts par source, `layers.3.mlp.experts.7-9.gate_proj`) :
  ...
  - noyau `nvfp4_dequant` : **non mesuré** (pas de carte) — même script, 1 min, Laure.
- **verdict** : **TENU** pour les deux références ; la prédiction de Sage est **réfutée par
