# poste7 — Clé `mlp.shared` ≠ `mlp.shared_expert` : portée rétroactive — les cellules mesurées restent vraies, une conclusion est affaiblie, et un contrôle à sec de 15 min dit quels convertis sont touchés (17/09)

Entrée : poste2 72ace16 (8e393ef, 0e4ef38) — la calibration AWQ attachait les statistiques de l'expert partagé sous l'attribut Python `mlp.shared.*` (`model.py:679`), `calibrate.py` les cherchait sous `mlp.shared_expert.*` : **l'expert partagé n'a jamais été calibré, il était arrondi au plus proche (RTN), silencieusement**. AWQ est le défaut de `acvram convert` (`cli.py:522`), donc chaque conversion d'un modèle à expert partagé passée par ce chemin porte le défaut.
...
## 3. Le contrôle qui rend « faux », à sec, 15 min (poste2)
...
* différents ⇒ touché ; le nouveau converti va à poste3 (PPL 3 tranches, 20 min). **Prédiction : 1,0150 → 1,009-1,014. Un seul seuil : ≤ 1,013 (au-delà de la résolution) ⇒ la colonne GLM acvram est remplacée par le converti corrigé ; > 1,013 ⇒ colonne inchangée, expert partagé insensible, la conclusion du § 2 est rétablie telle quelle.**
