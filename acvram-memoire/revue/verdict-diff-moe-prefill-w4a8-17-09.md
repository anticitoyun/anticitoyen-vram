...
Laurine, 17/09, arbre `laurine`, suite de verdict-bissection-w4a16-suite-17-09 (Laure 6df34fb : tout-torch 1,0104 contre noyaux 1,0280, ≥ 1,2 % non localisé) et de sage-bissection-w4a16-verdict-17-09 pt.3. Lecture seule + un instrument processeur ; aucune carte.
...
| experts routés : act | `ext.moe_act` fp32 silu, un arrondi bf16 (`acvram_kernels.cu:2842-2876`) | silu fp32 × u fp32 → bf16 (`model.py:1030-1032`) | ulp fp32 (fast-math), `tests/test_moe_glue.py` ; Laure : −0,19 % en masquant act+swiglu |
| experts routés : réduction top-k | `ext.moe_reduce_trie` somme fp32 pondérée, un arrondi (`acvram_kernels.cu:2905-2918`) | `d.float() × topw` → `sum` fp32 → bf16 (`model.py:1091-1092`) | ordre fp32 ; masqué par Laure : 1,0275 |
...
## Prédiction scellée (carte, Laure, 3 tranches privé, même instrument)
...
- entre 1,014 et 1,024 → une part, pas tout ; reste à chercher dans rmsnorm (−0,35 %, Laure) et la glue.
