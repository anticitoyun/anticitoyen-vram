# Pièce 77 — décomposer nos 0,842 ms/pas « hors noyaux » à b = 12, sans nsys — 23/09 (Océane)
...
## Verdict — 23/09 09 h 1x (Océane)

* **instrument** : `outils/gpu/mesure/hors-noyaux-p77.py` (témoin, boucle nue, compte des nœuds), `frontiere-pas.py 12 1024` (sans nsys) et `12 60` (sous nsys, **noyaux seuls**), `serve` + `banc-llamacpp-16-09.py decode` (comme la 64), `/metrics` en fin de fenêtre ; chaînes `scratchpad/oceane-p77-23-09/prise{1..5}.sh`
...
2. **Notre servi du jour est à 1 940-1 995 t/s** ; la 64 donnait 1 829-1 834 ce matin (vLLM 1 970-2 028). Je ne
   ...
   2,8 % entre eux. **Il faut rejouer l'ABBA de la 64 aujourd'hui** (Manon, même chaîne) avant de rouvrir la question
   de la porte b=12.
