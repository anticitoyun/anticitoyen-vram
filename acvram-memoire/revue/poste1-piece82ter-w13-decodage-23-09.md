# Pièce 82 ter — w13 au décodage seul, préfill séparé : critère TENU, w13 PAR DÉFAUT — 23/09 (poste1)

* **instrument** : `tests/test_moe_w13.py` (+ 2 tests : préfill par vues w13 au bit du séparé, T 1/16/256/3072 ; préfill court par le chemin tensor au bit, T 8/12/32, avec témoin négatif) ; `scratchpad/poste1-p82ter-23-09/empreinte-marlin.py` (sha256 de 12 GEMM Marlin sur entrées fixes, arbre d'avant contre arbre d'après) ; KL `kl-b.py` de la p81/p82 inchangé, ABBA dans une prise (W0 b12, W13 b12, W13 b1, W0 b1) ; `outils/gpu/mesure/capture-godets.py` (godets 1,2,8,16, warm_graphs) sur 7 alias MoE
...
* **verdict** : **critère TENU. w13 devient le défaut** (`ACVRAM_MOE_W13=1` ; témoin `=0`), sur ordre de chef. La 82 perdait la KL sur deux chemins que je n'avais pas séparés :
