# poste7 — affinité CPU dans les prises de T1 : OUI, régime `cpus0-15`, pour les trois moteurs (20/09, 10 h 35, horloge machine)

Question de chef (10 h 33, poste `20-09-1030`, ETAT e31c99be) : « le point fils n'est effectif que sous `carte.sh` avec `ACVRAM_CPUS` posé — affinité dans les prises de T1 ou pas (défaut : aucune) ? »

## Décision

**Oui : `ACVRAM_CPUS=0-15` dans toute prise de T1, concurrents compris.** Trois raisons, dans l'ordre :

1. Le lot demandé par l'utilisateur (« fils CPU à vérifier et régler ») est **un seul lot, un seul régime** (`poste7-poste-lot-20-09` § tableau, ligne fils : `OMP_NUM_THREADS=8`, affinité 0-15) ; l'ordre de retrait « fils → THP → C-states → ASPM » suppose les fils dedans. Un T1 sans affinité mesurerait un lot à quatre points et le cinquième n'aurait jamais de chiffre.
...
|---|---|---|
| GLM b=1 acvram (T1) contre M00-ter (même poste, **sans** affinité — poste2 l'a lancée 10 h 33 avant cette note : c'est le témoin gratuit) | pas −0,00 à −0,15 ms (0 à −2 %), \|A1−A2\| réduit | pas > M00-ter + 2·\|A1−A2\| → fils retiré |
...
`environment.d` n'est lu ni par GNOME ni par les shells (chef 10 h 33) → **`ACVRAM_CPUS=0-15` passé explicitement à `carte.sh` par chaque chaîne**, comme `ACVRAM_MODELES`. Dans chaque en-tête : ligne `hote=thp,omp8,cpus0-15` (acvram, `hote.py:94`) ; pour les concurrents `taskset -p` du PID servant et du client (`verifier.sh:20`) ; `ps -L | wc -l` du servant. Une cellule sans ces lignes n'entre pas dans INDEX.

## Ordre

* **poste2** — M00-ter et M1 bis se finissent **telles quelles** (sans affinité : M00-ter est le témoin « poste 1030 sans fils »). Puis T1 entier : `ACVRAM_CPUS=0-15` posé explicitement devant `carte.sh` pour **chaque** prise, acvram comme vLLM et llama.cpp ; en-tête : `hote=…,cpus0-15` (acvram) ou `taskset -p` du servant et du client (concurrents), `ps -L | wc -l`, `poste=20-09-1030`. Scellés : table ci-dessus ; < −3 % sur une cellule → retrait fils seul, contrôle 6 min, puis T1 continue.
* **chef** — `ACVRAM_CPUS=0-15` dans la ligne de régime de T1 dans ETAT ; `verifier.sh` relancé une fois sous charge pendant la première prise T1 (lecture seule, pas de carte) et sa sortie dans ETAT ; 0.6.31 embarque `outils/poste/` avec `ACVRAM_CPUS=0-15` documenté, non posé par le paquet.
* **poste1** — rien.
