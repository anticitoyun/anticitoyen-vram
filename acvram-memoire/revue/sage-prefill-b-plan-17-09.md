# Sage — A réfuté : la tuile n'est pas la cause, le nombre de GEMM l'est ; B s'attaque directement, en deux marches (B0 un lancement pour 128 experts, B1 lecture NVFP4 dans la tuile), sans enquête cuBLAS (17/09)

Entrée : Laure 0edc3b9 — `bmm` par pile 5 486 j/s contre `_grouped_mm` 8 614 (−36 %) : mêmes tuiles cuBLAS (M petit), 8 895 lancements au lieu de 13 902, et ≈ 58 Go de copies par `w[experts]`. PPL 1,0004 : la sortie n'a pas bougé, c'est bien un chantier de vitesse. Défaut `ACVRAM_PREFILL_GROUPED` remis à `grouped_mm` (`model.py:1581`) : juste, et le nom de régime doit rester dans `regime_ligne()`.
...
## 3. À écrire dans REGLES § 4 (Laurine + Laure, 17/09)
...
1. Jérôme : ETAT — A réfuté, B0/B1 scellés § 2, pas d'enquête cuBLAS ; REGLES § 4 entrée § 3 ; file noyaux B0 → B1 → E → C.
2. Laurine (à sec) : B0, un commit avec test d'équivalence et régime nommé ; passe de carte quand vert ; puis B1.
3. Laure : GLM calibA `CERT_PLAN_LEN` → poste E n'est pas à mesurer (c'est un noyau de Laurine, après B1) → bras KV lm4 → campagne ; passes B0/B1 dès prêtes.
