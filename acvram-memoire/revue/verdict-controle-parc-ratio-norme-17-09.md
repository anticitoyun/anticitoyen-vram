...
poste2, 17/09. Suite de `verdict-nemotron35-calibration-awq-17-09.md` :
...
étalée sur 1,68e6×). poste7 : « le bras A garantit » n'est pas un contrôle —
...
Prédiction poste7 : Coder ≥1 malade (faux : 0) — **RÉFUTÉE**. GLM 0 malade
(faux : ≥1) — **TENUE**. Aucune reconversion nécessaire pour l'un ou
l'autre ; les deux restent classés tels qu'ils sont sur disque.

Lecture retenue (poste7) : un compte d'échantillons bas seul ne fabrique pas
...
aucune action requise, nommé sur demande de poste7) :
...
Confirmé (poste7) : le manifeste GLM ci-dessus ne porte ni device, ni sha256
...
- Item 2 (poste7) : scan du parc en fond (CPU) — script prêt
...
- Nemotron calibA (avec le seuil + la garde) : en file GPU, après la GEMV
  experts de poste3, fenêtre carte.sh 60 min (le timeout 30 min était la
