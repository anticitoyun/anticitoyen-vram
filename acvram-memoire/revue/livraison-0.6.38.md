# Livraison 0.6.38 (Laure, 23/09, pièce 95)

Paquet construit sur main à jour (fusion de laure dans main jusqu'aux pièces citées ci-dessous).
Aucune installation lancée (`sudo dpkg -i` reste à l'utilisateur, feu explicite requis en plus
tant que la pièce 94 de Manon n'est pas tenue). Aucune publication GitHub.

## 1. Journal des changements depuis 0.6.37

### Pièce 85 — effondrement du service à lots successifs (Océane)
...
2. `/metrics` (donc toute supervision — tableau de bord, GUI) invalidait les captures en cours en
   ...
   `regime()` seulement).
Mesuré (`oceane-piece85-effondrement-23-09.md`, ABBA, alias Coder-30B-A3B-nvfp4-qkvo-i8c,
...
### Pièce 86 — b=2 sous b=1 en service (Océane)
...
### Pièce 87 bis — le −6,72 % de la pièce 87 n'est pas un effet du service à lots successifs (Manon)
...
### Pièce 88 — capture échouée : justesse des hybrides, puis tri des échecs (Océane)
...
### Pièce 89 — comparatif vLLM à ≥5 lots/bras, seuil statistique explicite (Manon)
...
### Pièce 82 ter — w13 au décodage seul, préfill séparé, PAR DÉFAUT (Océane)
...
### Pièce 92 — réduction d'attention déroulée, PAR DÉFAUT (Océane)
...
contredite par un ABAB SERVI antérieur (Manon, 20/09) — tranchée en service seulement.

### Pièce 90 — la ligne de régime des refus de graphe (Laure)
...
- Source : main à jour (worktree `laure`, fusionné jusqu'aux pièces 82 ter/85/86/87 bis/88/89/90/92).
...
- Installation (`sudo dpkg -i`) : à l'utilisateur, et **pas d'annonce « prêt à installer » avant
  que la pièce 94 de Manon (capture-godets sur main) soit tenue** — feu vert de Jerome attendu.
