# Pièce 85 — effondrement du service à lots successifs (Océane, 23/09)

Constat (prise invalide de la 82, `scratchpad/oceane-p82-23-09/banc-mint-T.log`) : un serveur unique, paliers
...
chaque seconde par `scratchpad/oceane-p85-23-09/releve-metrics.sh`. -lgc 2700. Prise ≤ 12 min, deux bras.

## Verdict

instrument : `scratchpad/banc-llamacpp-16-09.py` (HTTP/SSE, un serveur par bras, paliers 2→4→8→12 enchaînés) + `/metrics` toutes les 2 s, `scratchpad/oceane-p85-23-09/prise.sh`
...
Cause 1 (le constat de Jerome) : `acvram/engine/graphs.py:123` MAX_GRAPHS=16 sans éviction, et `graphs.py:614-618` refusait
...
graphe sur 10 942 et les captures en cours de palier (23 × 40-130 ms) n'expliquent pas tout — à mesurer par Manon, serveur
neuf contre lots successifs, même séance. (b) b=2 anormal dans les trois bras (99,8 à 247,5 t/s, sous b=1 = 311) :
spéculation n-gramme (lot_max 2) soupçonnée, non prouvée — pièce à ouvrir. (c) Cellule publiable = Manon (REGLES § 3,
