# AWQ par expert : gate et up à échelles distinctes dans la pile — 15/09 (Laurine)

Ordre de Jérôme (chantier 1) ; décision Sage (a) `sage-glm-gateup-16-09` ; témoin plancher
d'Océane conforme (2845b31) → **prêt à fusionner** (16/09 06:45). Contexte : `revue/awq-pile-15-09.md`, `revue/sage-glm-awq-pile-15-09.md`.
...
## Tests sur carte (16/09 06:27, laurine 5dcffdf)
...
## Contrôle du RÉFUTÉ d'Océane (dd51a3c) : c'est W4A4, pas l'échelle
...
|---|---|---|---|
| défaut (MIN_T=9, Océane) | 0-7 ok, 8-15 échec | 0,9-1,8 | coupure = `model.py:1340` `t >= MIN_T` |
...
peut pas être tenu par le chemin W4A4 par construction — c'est la table des trois issues de Sage
(`sage-glm-w4a4-16-09`) qui tranche : bogue (non), W4A4 coûte (à mesurer en PPL), métrique.

## Pas GLM b=12 (16/09, carte exclusive, `ACVRAM_TYPE=mesure`, fenêtres 25 s ABAB)
Scellé Sage (a) : B/A ≤ 1,03× (réfuté > 1,05×), coût attendu +0,1-0,3 ms/pas.
...
- Le scellé comparait deux convertis, pas deux codes : la part converti (+2,06 ms) n'est pas
  ...
  vérifier par Manon). La part échelles dépasse aussi l'attendu +0,1-0,3 ms (×3-10).
...
## Correctif quant_act (16/09) : 2^k fixe réfuté, échelle globale PAR LIGNE (Sage § 7)
- 04a9541 (k_x=4, k_act=8, gscales·2^-k) : re-PPL de Laure RÉFUTÉ (1,098 ; 56 k blocs saturés ;
...
- Les 25 s contre 6 s à k=(4,8) : pas expliqué par le code (aucune branche propre à « les deux
  non nuls » ; `_gs_mma` = un dict à 3 entrées par couche). Hypothèse à contrôler par Laure :
...
- t-qa 065960a (Laure) : ROUGE, 153 échecs, cause unique : `gr` du noyau ≠ référence sur ~20 %
...
(discriminateur d'Océane) ; montée 0.6.6 (MIN_T=5 déjà câblé bdb9f14, narrow OFF, re-tampon OFF
selon `revue/sage-narrow-verdict-16-09.md`).
