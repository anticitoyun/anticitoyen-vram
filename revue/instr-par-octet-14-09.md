# Instructions par octet : acvram vs vLLM au décodage b=12 (14/09, Laurine)

Commande de Jérôme sur l'avis de Sage (`acvram-memoire/revue/sage-strategie-14-09.md`) :
...
Réfutation (seuil de Sage) : ratio du pas entier ≤ 1,5× → la puissance
...
N=1, 29 passés). Prédiction scellée (Jérôme) : −1,2 ms/pas, −19 % d'octets.
...
d'Océane, à câbler ici avant toute reprise.
...
Conséquence pour Sage (« MoE 82 % de borne, aucun levier ») : faux sur
...
242 W nets de vLLM au duel venaient d'un repos plus élevé — Laure, même
...
Hypothèse de Sage : **confirmée, au-delà du seuil (×7,2 pour ≥ 3
...
3. **MoE en MMA au décodage** — le poste qu'on avait fermé (`GEMV reste`,
   ...
   ouvrir avec Jérôme : gate·up et down groupés en un lancement chacun,
...
## Étape (iii) de Sage — MoE en MMA groupée au décodage, b=12 eager (14/09, 19h51-19h58)

`ACVRAM_MOE_GROUPED_MAX=0`, `ACVRAM_MOE_MMA_BT=16` (une tuile m16 par
expert, M≈3 jetons par expert), eager, carte exclusive. Seuils de Sage
(`revue/sage-moe-mma-decodage-14-09.md`) : ≥ 5,0 ms ou ≥ 380 W → split-K.
...
< 5,0 ; la prédiction 4,0-4,3 de Sage est au point) ; **puissance
...
Selon la règle de Sage (≥ 380 W brut → split-K, +2 j, (3) derrière 1aj)
...
borne. Le pas complet sous graphes (Océane, (ii)) est la mesure qui
tranche `pas ≤ 380 W et ≤ 0,45 J/jeton`. Données :
`revue/donnees-ncu-ipo-acvram-mma-bt16-eager-14-09.txt`.

## Point 0 de reprise (Sage) — llama.cpp b=1 sous le même instrument (14/09, 10h10-10h34)

En-tête de mesure (REGLES §3) : 5090 seule (`CUDA_VISIBLE_DEVICES=0`),
plafond 400 W, horloge libre (3 135 MHz max ; sonde -lgc de Laure à
...
(celui de Katy/Laure), Coder-30B Q4_K_M (18,6 Go), `-np 1 -c 16384 -ngl 999` ;
...
Contre les seuils scellés de Sage (`sage-organisation-14-09.md`) :
...
* « octets ±10 % des nôtres » : **réfuté, +18 % chez nous** (2,30 vs
  1,95 Go) — l'écart est exactement celui de Laure sur J/jeton (−18 %), et
...
* « repos chaud 60-80 W comme nous » : **réfuté, 33,3 W** pour llama-server
  chargé (≤ 40 = « plancher » selon Sage) ; notre moteur en processus :
  42 W. Les 68-76 W de Laure sont ceux du serveur HTTP acvram, pas du
  moteur.
* « la différence est dans les noyaux, W nets ≥ 1,2× les leurs » :
  **réfuté dans l'autre sens** — leurs W nets sont ×1,26 les nôtres
  ...
  de Laure (1,168 vs 1,430) compare deux serveurs HTTP à repos différents
...
Manon (forward GPU de 2 couches hors verrou pendant le ncu acvram b=1,
10:20-10:25) : sans effet sur les COMPTES (octets, instructions), seules
les durées ncu — non utilisées — pouvaient bouger.

### Étape 2 (Sage) — bt=32 par noyau : sans objet, chiffré
