# AWQ par expert dans la pile MoE — 15/09 (Laurine)

Ordre : `revue/sage-glm-awq-pile-15-09.md` (Sage). Branche `laurine` a82bc28…fb40fe9.
...
**+0,12 ms (+0,9 %), 2,5 µs/couche** — au-dessus des 2 µs de Sage ; profil GPU d'un pas +0,07 ms
(route_pack +0,5 µs, moe_act +0,2 µs par couche), le reste au plafond de puissance.

## Garde d'unité (Sage § 8) et scellé Coder 0,00 ± 0,03 ms — RÉFUTÉ tel que scellé
Table = 1 partout (échelle absente écrite en identité, Manon 2205709) → `awq[nom] = None`, produit
...
## Non fait / à Sage
- Scellés PPL 0,998 ± 0,002 (Manon, GLM avec AWQ experts) et pas GLM b=12 ≤ 1,03× : pas de modèle
