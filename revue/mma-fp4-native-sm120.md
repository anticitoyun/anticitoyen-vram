...
Date : 13/09/2026 — Laurine, chantier 2. Test : `outils/test_mxf4_sm120.cu`.
Origine : campagne duck.ai (`acvram-memoire/revue/duck-laurine-13-09.md`),
...
n'est pas sous +1 % (Manon : +2,58 % sans lissage).
...
3. Ce que ce chiffre ne dit pas : la qualité. 5 888 j/s à +2,58 % de PPL
   ne se livre pas. Le lissage statique de Manon décide.
...
experts mesurée par Manon sur le noyau réel : PPL +0,919 % (seuil 1 %),
`ACVRAM_MOE_MMA=1` par défaut (Manon).

## Face à vLLM (14/09, soir) — décodage b=12, profil, tuile 128

Contexte : Manon a mesuré vLLM 0.29 sur la même carte, Coder-30B NVFP4 :
...
(le 16,54 datait d'une autre session de mesure, après le duel de Manon) ;
...
lignes par bloc), à mesurer par A/B propre. Bead à ouvrir si Jérôme le
retient devant 0si.
