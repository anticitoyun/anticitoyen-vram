# P1 — étalon FlashInfer b12x (SM120 W4A4 fused MoE) sur nos formes (15/09, Laurine)

Ordre de Sage (`sage-lancement-14-09.md` § 3) : le banc `bench_b12x_mxfp4_moe.py`
...
1. **Routage** : l'original route `(t·k + j) % E` → à 12 jetons top-8, 96
   ...
   de Sage (80-100 µs/couche à 12 jetons) suppose ~30 experts ; sous le
   routage d'origine il est inatteignable (255 Mo à 1 To/s = 255 µs).
   `--experts-pool 30` (Coder) / `--experts-pool 34` (GLM, calcul de Sage)
...
| forme, jetons, routage | Sage | moi (L2 froid) | moi (L2 chaud, banc d'origine) |
|---|---|---|---|
...
mise à niveau, décision Sage) ; (b) fusionner nous-mêmes gate·up + down
...
## Voie (c) de Sage — contrôle de dépendance du cubin (15/09, 1 h, hors carte)
...
Sage ne tient pas — sous réserve du point 3.
...
5. conversion de disposition des experts au chargement : `w13_fp4`
   ...
   chargement (Océane : placement, tables d'adresses — le `.o` prend des
...
Prédiction si (c) aboutit (rappel des seuils de Sage) : 62,7 µs ± 10 % à
froid avec fantômes ; pas Coder b=12 −1,8 à −2,3 ms sur le bras B de
c6377d5 ; J −12 % ; PPL b12x/A ≤ 1,010 (Manon, teacher forcing).
...
**60-75 µs/couche** (Sage : ≤ 75, réfuté > 90) contre 3 GEMM 75 + glue 30
...
(déterministe), FC2 contre float64 à tolérance, jetons A/B et PPL Manon.
...
Sage ordonne la marche de glue d'abord (quant dans l'épilogue de moe_act,
...
quant/act/reduce. Règle de Sage (≥ 20 → glue d'abord) : glue d'abord, et
...
Ordre de Sage (`sage-mma-lot-15-09.md` § 2) : le coût fixe de B hors glue
(2,0-2,3 ms/pas, courbe de Laure) serait un plancher de latence par GEMM
...
b=6 / b=1 = **2,31×** : le critère de Sage réfute son plancher tel quel
...
le reste du coût fixe de B à b=1 (2,0-2,3 ms mesurés par Laure) n'est PAS
...
(22 s), venv acvram, Coder-30B NVFP4, v0.6.3 + laurine 27ae762
...
Seuil de Sage (−0,6 à −1,0 ms, réfuté > −0,3) : **tenu et dépassé
...
Livré (laurine, `ACVRAM_MOE_DECODE_FUSED=1`, coupé par défaut) : CTA =
...
Le sériel coûte ≈ 45 µs/couche (seuil de Sage ≤ 5, > 10 → atomiques
...
Contre les seuils de Sage (§ 3) : **≤ 75 µs/couche froid : TENU (69,7)** ;
