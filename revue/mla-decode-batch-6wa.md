...
Date : 14/09/2026 — poste4. Spec : poste3,
...
- `MLAttention.decode_static_batch` : préparation (projections, RoPE, norme,
  ...
  `ext.mla_decode` est batché — périmètre retenu par chef (« le noyau seul,
...
Prédiction scellée (poste3) : −5,3 à −15,9 % sur `pas_total` ; réfutation :
< 3 %.
...
celui de poste3 sur la longueur des séquences (ici 32 + 150 jetons, godet
...
- ABBA complet en régime court (le dernier bras B a été tué par le
  ...
  poste1 puis poste3) : à relancer `ACVRAM_TYPE=mesure AB_TOURS=15
