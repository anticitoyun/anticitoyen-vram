...
|---|---|---|---|
| 解码 12 个序列 | **1634 t/s** | 1782 t/s | — |
| 解码 1 个序列 | **380,8 t/s** | 290,6 t/s | 323,6 t/s |
| prefill pp2048 | **22 707** token/s | 21 054 | 8 671（TabbyAPI，已撤回） |

当日吞吐（1030 号机，节能工况 `-lgc 2700`，服务中的流水线；贪心采样已捕获进 CUDA 图，自 0.6.35 起为默认）。b=12 是官方封存单元格（6 个交错窗口的中位数，逐窗口频率）。

> **勘误（22/09/2026）。** 0.6.35 的首次发布从 21/09 的一个 vLLM 基准 1596 t/s 得出「领先 vLLM +1,84 %」，而该基准来自一次**离线生成**（`LLM().generate()`），**无法与服务器相比**：没有连续调度，也不是 `acvram serve` 的路径。已于 22/09 用交替单元格 A/V（A1 V1 A2 V2 A3 V3）对阵 **`vllm serve`**（HTTP）纠正，同一张卡、与 `acvram serve` 相同的路径：vLLM 中位数 **1782 t/s**。在可比测量下，**acvram（1634 t/s）在 b=12 落后 vLLM 约 8 %**，并非领先。相同频率下的 J/token 仍在重新测量。
...
提供）；装不进显存的模型；以及单序列解码（b=1：380,8 t/s，对比 vLLM 的 290,6）。反之在大批次下，
装得进显存的 MoE 上，vLLM 在 b=12 仍然领先（1782 对比 1634 t/s，见勘误）；acvram 在此有进步
（0.6.34 的 1540 → 1634），但未反超。能耗上的差距尚待重新测量。
