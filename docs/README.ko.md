...
한 6개 창의 중앙값, 창별 클럭)이다.

> **정정(2026/09/22).** 0.6.35의 최초 공개는 「vLLM 대비 +1,84 %」를 09/21의
> vLLM 기준 1596 t/s에서 끌어냈지만, 이는 **오프라인 생성**
> (`LLM().generate()`)에서 나온 것으로 **서버와 비교할 수 없다**: 연속 스케줄링
> 없음, `acvram serve` 경로도 아님. 09/22에 A/V 교대 셀(A1 V1 A2 V2 A3 V3)로
> **`vllm serve`**(HTTP)에 대해 같은 카드·같은 경로(`acvram serve`와 동일)로
> 보정했다: vLLM 중앙값 **1782 t/s**. 비교 가능한 측정에서 **acvram(1634 t/s)은
> b=12에서 vLLM보다 약 8 % 뒤**이며 앞이 아니다. 동일 클럭 J/토큰은 재측정 중.
...
단일 시퀀스 디코딩(b=1: 380,8 t/s 대 vLLM 290,6). 반면 대배치에서는 VRAM에
들어가는 MoE에서 vLLM이 b=12에서 계속 앞선다(1782 대 1634 t/s, 정정 참조);
acvram은 전진했으나(1540〔0.6.34〕→ 1634) 앞서지는 못했다. 에너지 격차는
재측정이 필요하다.
