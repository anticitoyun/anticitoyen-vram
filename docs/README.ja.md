...
|---|---|---|---|
| デコード 12 シーケンス | **1634 t/s** | 1782 t/s | — |
...
> **正誤（2026/09/22）。** 0.6.35 の最初の公開は「vLLM 比 +1,84 %」を 09/21 の
> vLLM 参照 1596 t/s から導いていたが、これは**オフライン生成**
> （`LLM().generate()`）由来で、**サーバーとは比較できない**——連続スケジューリング
> なし、`acvram serve` の経路でもない。09/22 に A/V 交互セル（A1 V1 A2 V2 A3 V3）で
> **`vllm serve`**（HTTP）に対し、同じカード・同じ経路（`acvram serve` と同一）で
> 補正した: vLLM 中央値 **1782 t/s**。比較可能な測定では、**acvram（1634 t/s）は
> b=12 で vLLM の約 8 % 後方**であり、前ではない。クロック同一の J/トークンは
> 再測定中。
...
モデル、そして単一シーケンスのデコード（b=1: 380,8 t/s に対し vLLM は 290,6）。
一方、大バッチでは、VRAM に収まる MoE 上で vLLM が b=12 で先行し続ける
（1782 対 1634 t/s、正誤参照）。acvram は前進した（1540〔0.6.34〕→ 1634）が
前には出ていない。エネルギーの差は再測定が必要。
