...
def test_le_temoin_copie_topi_et_garde_son_adresse(monkeypatch):
    monkeypatch.setattr(MOE, "_ROUTAGE_TEMOIN", True)
    monkeypatch.setattr(MOE, "_TEMOINS_ROUTAGE", {})
    ...
    b = Bloc()
    src = MOE.MoEBlock.forward.__code__   # le code existe ; on rejoue la logique du témoin
    for topi in (torch.tensor([[1, 2], [3, 4], [0, 0]], dtype=torch.int32),      # préfill : une autre forme
        ...
        if tm is None or tm.shape != topi.shape:
            ...
            MOE._TEMOINS_ROUTAGE[id(b)] = tm
        tm.copy_(topi)
    assert len(MOE.temoins_routage()) == 1 and torch.equal(MOE.temoins_routage()[0], torch.tensor([[5, 6], [7, 8]], dtype=torch.int32))
