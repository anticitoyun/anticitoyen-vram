...
def test_le_plan_ne_reserve_pas_de_seconde_disposition(monkeypatch):
    """Disposition unique (sage-p1-disposition-unique-18-09) : la réserve du
...
def _reference_fp32(bloc, x, topw, topi):
    ...
    temps sous un critère relatif près de zéro : hypothèse (a) de Sage,
    ...
    (Sage, 18/09 : « la référence passe son propre critère »)."""
...
# à sec, T = 1 024) — la référence doit passer son propre critère (Sage), le
...
def test_marlin_et_groupe_contre_fp32_et_le_bras_casse(monkeypatch, awq, T):
    ...
    passait devant Marlin et un test comparait sans l'atteindre — Laure
    ...
    # roll_cuda n'existe pas pour Float8_e4m3fn — Laure 5803c1e)
...
def test_le_critere_a_sec_deux_approximations_bf16_ne_se_comparent_pas_entre_elles():
    """Hypothèse (a) de Sage, tranchée par émulation torch (CPU) : B0
    ...
    Marlin-contre-B0 ≈ 3 % (ce que Laure a mesuré, 3,7-4,3 %) ; sous
    ...
    laure = lambda y, r: ((y - r).abs() > 2 ** -7 * (r.abs().mean() + r.abs())).float().mean().item()
    assert laure(b0, ref) > 0.01 and laure(marl, b0) > 0.01, (laure(b0, ref), laure(marl, b0))   # le critère relatif condamne B0 lui-même
...
def test_a_sec_la_reference_passe_son_propre_critere_avec_awq(monkeypatch, awq, T):
    """Les 4 cas de la carte, émulés à sec (Sage : « 4/4 verts avant que Laure
