...
def test_la_bascule_triton_porte_sa_mesure_et_le_regime_l_imprime(monkeypatch):
    """Condition (b) de Sage (sage-e-c-verdict-17-09 § 2) : la constante de
    bascule est le plus petit lot où Triton bat CUDA dans la mesure de Laure
...
def test_la_derniere_tranche_ne_lit_pas_au_dela_des_echelles():
    """Laure b34a1bb : accès mémoire illégal dans le moteur (b = 12, capture
...
def test_le_noyau_ne_lit_rien_au_dela_du_godet_ni_des_poids(godet):
    """Condition (1) de Sage (sage-e-c-verdict, remesure b34a1bb, REGLES § 3) :
