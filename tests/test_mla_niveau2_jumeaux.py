"""C15 niveau 2 (sage-c15-niveaux-20-09 § Ordre 04 h 20 : « jumeaux torch par noyau à sec,
...
@pytest.mark.parametrize("graine", [3, 11, 29])          # trois graines (Sage 13 h 28) : la marge −0,00 tient-elle ?
def test_chaque_noyau_contre_son_jumeau_sur_carte(monkeypatch, graine):
    ...
    torch, au juge § 7 (sage-t4-tri-69-20-09 addendum 12 h 20) : deux approximations bf16 ne se
    ...
    with torch.inference_mode():
        ...
        # entre deux ordres de somme légitimes du même calcul en fp32, Sage 13 h 12 : ni 6 ni 32·eps32) ;
...
def test_remede_regles_6_caches_paresseux_refuses_en_capture(monkeypatch):
    """Remède C15 niveau 2 (sage-glm-b1-noeuds-c15-niveau3 § 2, REGLES § 6) : tout cache paresseux
...
def test_rejeu_simule_avec_un_pas_different_change_la_sortie(monkeypatch):
    """REGLES § 6 (Sage, addendum niveau 2) : tout compteur par pas passe par un tenseur sur carte —
