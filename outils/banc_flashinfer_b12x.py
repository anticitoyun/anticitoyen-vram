...
--- Copie adaptee pour acvram (poste4, 15/09/2026, P1 de poste7,
revue/poste7-lancement-14-09.md section 3) ; source : flashinfer 0.6.18.post1,
benchmarks/bench_b12x_mxfp4_moe.py, licence Apache-2.0 ci-dessus. Ajouts :
...
def main():
    ...
    parser.add_argument("--experts-pool", type=int, default=0,
                        help="routage sur N experts distincts au plus (0 = routage d'origine, t*k distincts)")
    parser.add_argument("--cold-l2", action="store_true", help="purge du L2 entre les mesures")
    parser.add_argument("--repos-s", type=float, default=10.0, help="repos NVML avant (s)")
    ...
    print(
        "quant_mode,tokens,experts_distincts,backend,latency_ms,tflops,W_brut,W_net,repos_W,MHz,bridages,cold_l2",
        flush=True,
    )
    for num_tokens in args.tokens:
        ...
        if args.experts_pool:
            g = torch.Generator(device="cpu").manual_seed(args.seed + num_tokens)
            pool = min(args.experts_pool, args.num_experts)
            token_ids = torch.stack([torch.randperm(pool, generator=g)[: args.top_k]
                                     for _ in range(num_tokens)]).to("cuda", torch.int32)
        else:
            token_ids = (
        ...
        distincts = int(token_ids.unique().numel())
        ...
        for quant_mode in args.quant_modes:
            ...
            print(
                f"{quant_mode},{num_tokens},{distincts},{backend},"
                f"{latency_ms:.6f},{achieved_tflops:.2f},{e.moyenne:.1f},{e.moyenne - e0.moyenne:.1f},"
                f"{e0.moyenne:.1f},{max(horloges) if horloges else 0},{'+'.join(sorted(e.bridages)) or '-'},{int(args.cold_l2)}",
                flush=True,
            )
