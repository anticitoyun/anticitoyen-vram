#!/usr/bin/env python3
"""Duel A2 (audit Sage, item A2) : duel à TROIS moteurs — acvram, llama.cpp,
vLLM. TabbyAPI retiré le 15/09 (décision de Sage : hors objectif B, ×2,5-8
...
de Jérôme (14/09) : valider les rivaux sur Qwen3-Coder-30B-A3B, régime
pp2048, même NVML
power.draw.instant (idle soustrait), même fenêtre — contre notre chiffre
du jour (acvram, prefill 16 938 j/s, Laurine 14/09).

Dénominateur COMMUN (Laurine, 14/09, après ma première mesure buguée —
...
implémentent CE protocole, chacun dans le venv de son moteur :
  outils/banc_prefill_chaud.py   (acvram, branche laurine ea98f36)
  outils/banc_prefill_vllm.py    (vLLM, même dénominateur, écrit ici)
  outils/banc_llamacpp_reel.py   (llama.cpp, binaire réel sm_120 de Katy,
                                  même dénominateur, Manon 14/09)
...
def main() -> int:
    ...
    print(f"BEAD audit Sage A2 — duel 3 moteurs, moteur={a.moteur}")
    print(f"  pp {PP_LEN}, GPU {GPU}, denominateur commun "
          f"(Laurine 14/09) : generate(max_tokens=1), invite differente "
