"""Micro-banc GEMV bf16, préparé sur demande de Jérôme (9/09), NON EXÉCUTÉ —
la 5090 est à Manon. À lancer une fois la carte rendue.

Corrigé après revue de Manon (`acvram-memoire/revue/metriques-ncu.md`) :
mes métriques et mon diagnostic d'interpréteur étaient faux, voir ci-dessous.

**Portée revue à la baisse, à dire honnêtement** : Manon a mesuré le vrai
...
le L2). Mon écart de conception gate/up séparés (voir laurine.md) **ne
...
appellent pour un poids non quantifié — voir laurine.md §"Chemin de lecture
...
par Manon ET par moi, pas supposé) : `anticitoyen-vram/.venv/bin/python`.
Mon message précédent disait l'inverse — inversion corrigée ici.

Usage prévu, SOUS LES MÊMES INSTRUMENTS QUE MANON :
...
Lire le CSV avec `scratchpad/lire-ncu.py` de Manon (les deux pièges de
...
HIDDEN = 5120        # forme du comparatif : à recaler si Jérôme/Manon
...
def bench(fn, *args, passages=PASSAGES, chauffe=3, nvtx_nom=None):
    ...
    # trafic DRAM (regle de Manon, metriques-ncu.md).
