...
pour le duel A2 (bead audit Sage) : « même NVML power.draw.instant, idle
soustrait, même fenêtre » pour les quatre moteurs — protocole de Jérôme,
14/09/2026.
...
def mesurer_idle(gpu: int = 0, secondes: float = 3.0, pas: float = 0.2) -> float:
    ...
    Moyenne, pas médiane (Sage, 14/09) : sur une charge par rafales, la
...
def mesurer_pendant(fn, gpu: int = 0, pas: float = 0.1):
    ...
    (Sage, 14/09 : +51 % d'écart mesuré entre médiane et compteur NVML
