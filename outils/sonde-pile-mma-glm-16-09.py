#!/usr/bin/env python3
"""Sonde par étape de la pile MMA GLM (ordre poste7, poste7-glm-pile-correctif-16-09 § 3.4).

Pile réelle (`MoEBlock._forward_grouped_mma`, MMA=1) de la couche MoE 1, sur
les 16 jetons et les 8 experts d'poste1 (x réel capturé sur le mini-répertoire
...
def _x_reel(n: int) -> torch.Tensor:
    ...
    if n != d.N_JETONS if hasattr(d, "N_JETONS") else False:
        raise SystemExit("--jetons doit valoir les 16 jetons d'poste1 (même x)")
