#!/usr/bin/env python3
"""Audit de Sage (revue/sage-audit-connaissances-14-09.md, item A1) : TTFT
...
# Régime du duel du 13/09 (laurine.md:3739-3750) : invites de ~350 mots de
...
def _mesurer_concurrent(engine, tokenizer, a) -> int:
    ...
    print(f"  concurrence {N} : {sum(len(p) for p in prompts_ids)} jetons prompt total, "
          ...
          flush=True)
    print(f"\n{'CONFIRME >70 %' if pct_hors > 70 else 'INFIRME <=70 %'} — cf. audit Sage")
...
def main() -> int:
    ...
    ap.add_argument("--invites-longues", action="store_true",
                    ...
                    "laurine.md:3739-3750) au lieu des invites-jouets courtes")
    a = ap.parse_args()

    print("AUDIT Sage A1 — TTFT Qwen3-Coder-30B-A3B-nvfp4, decompose par etape")
    ...
    print(f"\nmediane hors-forward : {med:.1f} %  "
          f"({'CONFIRME >70 %' if med > 70 else 'INFIRME <=70 %'} — cf. audit Sage)")
