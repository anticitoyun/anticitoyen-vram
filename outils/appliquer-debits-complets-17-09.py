#!/usr/bin/env python3
"""
Applique les débits b=1 du TSV fiche-service aux menus.
CSV : nom_menu, débit_b1, moteur
"""
...
def load_mapping(csv_path):
    """Charge le CSV des débits."""
    mapping = {}
    with open(csv_path) as f:
        reader = csv.DictReader(f)
        for row in reader:
            model_name = row['nom_menu'].strip()
            if model_name not in mapping:
                mapping[model_name] = {
                    'debit_b1': row['debit_b1'].strip(),
                    'moteur': row['moteur'].strip(),
                }
    return mapping
...
def add_debit_to_line(line, model_name, mapping):
    ...
    # Ajouter le débit (sans source de verdict pour TSV, juste moteur)
    debit_info = f"| b=1 {m['debit_b1']}"
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 appliquer-debits-complets-17-09.py <mapping.csv> [--dry-run]", file=sys.stderr)
        sys.exit(1)
