#!/usr/bin/env python3
"""Chiffre certifie b=12 mode moyen (poste7, poste7-poste3-15-09 § 3) : un bras
...
# Bras témoin du protocole ABAB de poste3 (protocole-certifie-smi-abab-18-09,
# relayé par chef) : reproduit l'ancien instrument (sous-processus
# `nvidia-smi` dans la fenêtre) pour comparaison, un run sur deux. Le défaut
# (CERT_SMI_TEMOIN absent) est déjà le correctif de poste7-profil-verdict-18-09
...
def smi(champs):
    ...
    par appel, celui-là même dont poste7-profil-verdict-18-09 §3 a mesuré le
...
class GardeSmi:
    ...
    corrélé à la variable étudiée », poste7-profil-verdict-18-09 §3 : un
...
# (19/09, poste7) certifie ne pose plus AUCUNE variable de régime que le serveur ne pose pas :
...
engine = Engine(loaded, None, max_batch_size=B, max_model_len=CTX,
                enable_cuda_graphs=os.environ.get("CERT_EAGER", "0") != "1")   # CERT_EAGER=1 : sans graphes (poste7 11)
...
if PUR:
  ...
  with GardeSmi() as garde_smi, Energie() as e:
    ...
    torch.cuda.synchronize()
    duree_hote = time.perf_counter() - t_hote
# Garde structurelle (poste7-profil-verdict-18-09 §3, REGLES §4) : hors bras
...
if garde_smi.n and not CERT_SMI_TEMOIN:
    print(f"[CERT {BRAS}] REFUS : {garde_smi.n} appel(s) nvidia-smi lance(s) "
         ...
         f"(poste7-profil-verdict-18-09 §3).", flush=True)
...
# poste7-poste-d-verdict-17-09 §2) : une sequence tronquee par budget KV epuise
