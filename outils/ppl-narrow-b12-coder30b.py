#!/usr/bin/env python3
"""PPL teacher-forcing À b=12 — voyant `narrow_gemm` corrigé (Sage, revue/
sage-narrow-b12-16-09.md, 16/09, amendant le b=1 de `ppl-decode-mma-
...
NE prend PAS narrow — piège nommé par Sage).

TROIS mesures, un seul montage, trois bras (A, B, A-eager) :

1. PPL teacher-forcing B/A, seuil scellé (Jérôme) **1,000 ± 0,002**.
...
3. Taux de divergences top-1 (argmax du modèle, PAS le jeton forcé) sur les
   ...
   scellé **taux(B vs A) ≤ 1,2 × taux(témoin)** — règle le s3@3 de Laure
...
def main() -> int:
    ...
    # PPL_DECALAGE (jetons) : tranche disjointe du corpus (Sage narrow-verdict § 3 :
