#!/usr/bin/env python3
"""verifier-contexte — la colonne ctx des menus contre le modèle et contre le plan acvram (sage-menus-test-reel-contexte-20-09 § 3a).
...
def ctx_plan(chemin: Path, colonne: int, acvram: str, timeout: int = 600, profile: str = "") -> tuple[int | None, str]:
    ...
    # à sec, la carte est cachée : sans --profile le plan ne voit aucune carte et rend kv_max_tokens = 0 (Femoceane 21/09,
    # 161 alias « RÉDUIT ») ; le profil déclaré planifie pour la machine, la carte reste à Manon
...
def ctx_servi_par_alias(journal: Path | None = None) -> dict[str, int]:
    ...
    (sage-s2-k48-feu-vert-21-09 § 2b(c)) : une colonne ne portant pas cette valeur est FAUX,
            ...
            for l in src_lignes:
                ...
                if l and not l.startswith("#") and len(c) >= 3 and c[0] in lignes:
                    ...
                    if cible is not None and str(cible) != c[2]:
                        # sage-3b-lanceur-contexte-20-09 : une colonne posée par le plan, pas par une requête tenue,
