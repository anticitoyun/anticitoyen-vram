...
point (2) — Sage §4, `revue/sage-cache-experts-13-09.md`).

Ce fichier ne construit PAS encore la table (ça attend l'intégration avec le
noyau MMA de Laurine) : il fixe le contrat que le noyau consomme et que le
Python remplit, pour que les deux côtés avancent sans se coordonner à chaque
ligne. Écrit par Océane le 13/09/2026, envoyé à Laurine le même jour.
...
def construire_table(experts: list, projection: str,
    ...
    `outils/test_uva_pin_memory.py`, confirmé indépendamment par Laurine sur
