...
def taux_de_succes_par_couche(chemin_journal: str, capacite: int,
    ...
    revue/sage-cache-experts-13-09.md §5, point manquant §3) : « si CETTE
...
def taux_de_succes_pin(chemin_journal: str, capacites: list, entrainement: float = 0.5) -> dict:
    ...
    l'a appris (Sage §5 : « jamais sur la moitié qui l'a appris »).
...
# ---------------------------------------------------------------------------
# M1 étendu (sage-c9-119b-cache-experts-19-09 § 2) : ce que la trace doit
...
def rapport_m1(chemin_journal: str, capacites=(16, 32, 48, 64, 96), entrainement: float = 0.5,
    ...
    critère de Sage : Δh = h(E/2) − 0,5 (E = nombre d'experts vus) — < 0,10
    ...
    return {"experts_vus": E, "lignes": n_lignes, "capacites": list(capacites),
            ...
            "verdict_sage": ("cache engagé" if meilleur - 0.5 >= 0.25 else
                             "pas de cache apprenant" if meilleur - 0.5 < 0.10 else "bande 0,10-0,25 : Sage tranche")}
