"""Attention de décodage paginée INT8 en Triton — poste E (sage-poste-d-verdict,
sage-b0-et-cause-lm4-17-09) : `paged_attn_partial_kernel` (acvram_kernels.cu:891)
...
if triton is not None:
    ...
    def _tranche(q, kc_ptr, ks_ptr, vc_ptr, vs_ptr, tables_ptr, b, hkv, N, debut, fin, debut_page, scale,
        ...
        for t0 in range(debut_page, fin, BN):
            ...
            # 2⁻⁸ du noyau CUDA, Laure) ; la somme reste fp32
    ...
    def _partiel_kernel(q_ptr, kc_ptr, ks_ptr, vc_ptr, vs_ptr, tables_ptr, lens_ptr,
        ...
        # du module dans un @jit (l'interpréteur l'acceptait — Laure, banc E)
        b = tl.program_id(0)
        hkv = tl.program_id(1)
        c = tl.program_id(2)
...
# C15-3d : 8 warps = 103 registres/fil et occupation 24 % (Manon 04 h 40, contre 178 et 11 % à 4),
...
WARPS_COMPACT = int(os.environ.get("ACVRAM_ATTN_WARPS_COMPACT", "8"))   # 8 : B8 +11,5 %, J 0,966 × ; B4 +9,1 %, 0,986 × et 369 W = 369 (Manon 05 h 15)
