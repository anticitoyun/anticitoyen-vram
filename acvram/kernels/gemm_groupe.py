...
(sage-prefill-b-plan-17-09 § 2) : UN lancement pour tous les experts.

Le coût du prefill MoE n'est ni la synchronisation ni la tuile (A réfuté,
Laure 0edc3b9) mais 3 × 128 GEMM séquentielles par couche, chacune trop
...
if triton is not None:
    ...
    def _gemm_groupe_nvfp4_kernel(xs_ptr, qw_ptr, bs_ptr, gs_ptr, y_ptr, te_ptr, t0_ptr, tn_ptr,
        ...
        (sage-b1-verdict-17-09 : la version arithmétique fp32 par valeur
...
# (Laure cb7bfa2). B1' (NVFP4) : l'opérande B pèse 4 + 0,5 Kio par étage au
