...
QUANT_CONFIGS = [
    # port acvram (23/09, pièce 101) : NVFP4 seul — E2M1 + échelle E4M3 par bloc de 16, activation bf16
    {
        "a_type": ["kBFloat16"],
        "c_type": ["kBFloat16"],
        "b_type": "kFE2M1f",
...
def remove_old_kernels():
    for filename in glob.glob(os.path.dirname(__file__) + "/dense_*kernel_*.cu"):
...
def generate_new_kernels():
    ...
    for result_dict_tmp in [result_dict, sm_75_result_dict]:
        for (a_type, b_type, c_type), config_list in result_dict_tmp.items():
            ...
            filename = "dense_" + filename.lower()      # port acvram : le MoE a le même nom d'objet
