...
VARIABLES_LUES = {
    ...
    "ACVRAM_ROPE_KV",
    "ACVRAM_NORME_FUSEE",
    # Deux interrupteurs de diagnostic (sage-kv-lm4-clos-17-09 § 1), lus par
    # memory/kv_lm4.py::actif / hors_puits, pas par le noyau ou le chargeur :
    # la passe de cause de Laure (K seul, V seul, puits) decide seulement si
    # tq3+1 s ecrit apres le commit B de Laurine.
    "ACVRAM_KV_LM4_SEUL",
    "ACVRAM_KV_LM4_PUITS",
    ...
    "ACVRAM_GLUE_COMPACT", "ACVRAM_GLUE_COMPACT_ITEMS", "ACVRAM_ATTN_WARPS_COMPACT", "ACVRAM_ATTN_REDUC_DEROULEE", "ACVRAM_ROUTAGE_TEMOIN",
...
def _doctor_eco(cuda_ok: bool) -> None:
    """Éco par défaut (sage-eco-2700-defaut-19-09 § 4) : le droit sudo ET
...
def _calib_source(use_awq: bool, calib_file: Optional[str]) -> dict:
    ...
    Sage (`sage-corpus-16-09.md` § 8) : le manifeste ne portait jamais QUEL
...
def cmd_convert(args: argparse.Namespace) -> int:
    ...
    # sage-diff-octet-a-octet-retire-17-09, REGLES §4 : sans le commit du
    ...
    opts = ConversionOptions(
        ...
        # ce que load_calib_ids a REELLEMENT rendu (Sage, sage-calibration-
    ...
    if report.tenseurs_replies and not args.dry_run:
        # sage-awq-relu2-garde-repli-17-09, REGLES §4 : le regime (combien
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    # Ordre imposé (Maîtresse 21/09) : plan → clamp → capture → chauffe. Le contexte demandé se PROUVE au chargement
    # (sage-3b-lanceur-contexte-20-09 (ii)) par un prefill plein dans le régime servi, graphes masqués ; le contexte
...
def cmd_eco(args: argparse.Namespace) -> int:
    """Mode eco d'horloge (sage-e1-eco-tenu-19-09 § 2, sage-eco-2700-defaut-
...
def build_parser() -> argparse.ArgumentParser:
    ...
    cv.add_argument("--alpha-commun-experts", action="store_true",
                    help="le meme alpha AWQ commun, mais PAR EXPERT MoE : "
                         ...
                         "(revue/oceane-disposition-naturel-qkv-22-09.md). "
    ...
    cv.add_argument("--alpha-commun-gate-up", action="store_true",
                    help="item A7 (audit Sage, 14/09) : un SEUL alpha AWQ "
    ...
    cv.add_argument("--hadamard-experts", action="store_true",
                    help="tourne les poids d'experts (gate/up/down) en "
                         ...
                         "echelle par canal ne corrige (sage-hadamard-16-"
                         "09.md). Defaut faux")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        ...
        if getattr(args, "func", None) is cmd_serve:
            ...
            # « size of tensor a (256) must match b (257) » sans site (P3 (4) 30B, Manon 21/09) — la pile s imprime
            import traceback
