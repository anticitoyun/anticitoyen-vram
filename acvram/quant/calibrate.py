...
def fwht_activations(x: torch.Tensor, block: int) -> torch.Tensor:
    ...
    (nvfp4_quant_act, sage-hadamard-16-09) : étages papillon en fp32 (chaque
    ...
    # pendant une capture de graphe CUDA (Manon, verdict-glm-hadamard-conversion :
...
def _magnitude_avec_plancher_relatif(mean_abs: torch.Tensor) -> torch.Tensor:
    ...
    une statistique de position comme la médiane (sage-awq-plancher-
...
def search_channel_scales(
    ...
    ``quantize_activation_nvfp4`` (Sage, `sage-glm-mma0-verdict-16-09.md` § 2,
...
def search_channel_scales_commun(
    ...
    Item A7 de l'audit Sage (14/09) : `search_channel_scales` appelee
...
def kld_couche_bits(y_ref: torch.Tensor, y_q: torch.Tensor) -> float:
    ...
    Duck.ai (Manon, 12/09/2026, revue/duck-manon-12-09.md) : trois modeles a
    ...
    C'est un OPTION AJOUTEE, pas un remplacement du SNR (consigne Jerome
...
def quantize_with_calibration(
    ...
    veut un bloc PLUS PETIT que celui-ci (Sage, `sage-hadamard-16-09.md` :
    ...
    metrics = {
        ...
        # sage-awq-experts-peu-routes-portee-17-09 : ‖poids reconstruit‖ /
    ...
    if mesurer_kld:
        ...
        # decision du convertisseur (consigne Jerome 12/09). Ce champ n'est
