...
class ConversionOptions:
    ...
    # les ait produits — sage-calibration-verdict-17-09).
    calib_tokens: int = 0
    calib_seqs: int = 0
    ...
    awq: bool = True
    group_size: int = 128
    # Q1 (Maîtresse 21/09, arXiv 2512.02010) : règle d'échelle de bloc nvfp4, max6 | 4sur6 — posée sur le module
    ...
    # sage-p2-qkvo-int8-canal-18-09 : q/k/v/o restent int8, mais symetrique
    ...
    # sur le processeur (Manon 21/09 : 31B `--quant-device cpu`, 96 min par
    ...
    # vide = toutes. Bead 1aj décodage (Sage, 14/09) : sur Coder-30B tous les
    ...
    # `out_snr_db`. Sert au protocole A/B (revue/duck-manon-12-09.md) qui
    ...
    # Item A7 de l'audit Sage (14/09) : `gate_proj` et `up_proj` lisent la
    ...
    # 22/09 (`oceane-disposition-naturel-qkv-22-09`) : le MÊME alpha commun,
    ...
    # Sage (`sage-hadamard-16-09.md`, 16/09) : la métrique W4A4 des experts
    ...
    # que le converti serve exactement les poids de vLLM (sage-convertisseur-
    # formats-16-09 § 3.1). Les couches gardées en clair par la source
    # suivent le plan comme avant.
    passage_direct: bool = False
    # Sage (`sage-corpus-16-09.md` § 8) : nom + sha256 du fichier de
...
class ConversionReport:
    ...
    # ambigue) -- voir la boucle principale et revue/sage-glm-awq-
    # pile-15-09.md (Sage a6a7436 : echelle AWQ par expert gardee dans la
    # pile, portee cote loader par Laurine).
    ...
    # sage-awq-relu2-garde-repli-17-09 : tenseurs dont l'AWQ a ete rejete
...
# Tour visuelle (contrat sage-go-multimodal-organisation-20-09 § 2, pièce (a)) :
...
# Qwen3-VL (contrat sage-go-qwen3vl-parallele-20-09 § 2) : les fusions deepstack
...
def _est_projection_attn(name: str) -> bool:
    """sage-p2-qkvo-int8-canal-18-09, generalise le 18/09 pour GLM (MLA) :
...
class TensorRouter:
    ...
    def format_for(self, name: str) -> str:
        ...
        if self.spec.model_type == "nemotron_h" and (
            ...
            # sage-hybrides-etape1-close-gemm-dense-17-09 : le checkpoint
...
def _nemotron_h_rename(name: str, n_layers: int) -> Optional[str]:
    ...
    `collect.py` (calibration AWQ) -- sage-hybrides-etape1-close-gemm-dense-
...
def _verifier_homogeneite_moe(tensors: dict, num_layers: int) -> None:
    ...
    Sage, 15/09 (`revue/sage-glm-formats-mixtes-15-09.md` §2, précisée par
    `revue/sage-glm-awq-pile-15-09.md`) : l'homogénéité de FORMAT d'une pile
    ...
    L'HOMOGÉNÉITÉ D'ÉCHELLE AWQ N'EST PLUS EXIGÉE ICI : Sage (a6a7436) a
    tranché que l'échelle AWQ par expert reste légitimement hétérogène dans
    la pile — c'est au chargeur (Laurine, échelle `[E,K]` portée après
...
def _convertisseur_commit() -> Optional[dict]:
    ...
    # (manon, laure, laurine, ...), la norme ici, pas l'exception.
...
def _sha256_du_checkpoint(chemin: str) -> Optional[str]:
    ...
    liste triée `nom:sha256`) -- sage-diff-octet-a-octet-retire-17-09,
...
def refus_nvfp4_sans_gpu(plan: Any, out_dir: str, format_impose: Optional[str]) -> Optional[str]:
    """Garde (Maîtresse 21/09, `oceane-hypotheses-qvl-30b-21-09`) : un nom de sortie ou un format demandé qui dit
...
def convert_checkpoint(model_path: str, plan: Plan, opts: ConversionOptions,
    ...
    manifest: dict[str, Any] = {
        "acvram_version": 1,
        # sage-expert-partage-cle-portee-17-09, REGLES §4 : un correctif du
        ...
        # sage-diff-octet-a-octet-retire-17-09 (REGLES §4) : la recherche AWQ
        ...
        # sage-p2-qkvo-int8-canal-18-09 : nom du regime d'attention porte au
    ...
    per_layer: list[dict] = []
    keys = []
    # sage-awq-experts-peu-routes-portee-17-09 : garde permanente, pas
    ...
    # sage-awq-plancher-median-faute-18-09 : le nombre qui relie directement
    ...
    # sage-awq-relu2-garde-repli-17-09, geste (b) : repli identite PAR
    ...
    # Sage (gate!=up, main 7c3698d) : `_try_build_stacks` ne force plus
    ...
    for name, tensor in _adapt_hf(_iter_checkpoint(model_path, opts.passage_direct), spec):
        ...
        if (source_quantifiee and tensor.dim() == 2 and tensor.dtype in (torch.bfloat16, torch.float16)
            ...
            # servirait d'autres poids que vLLM (Laure, 17/09 : q_a/q_b/kv_a/
        ...
        # sage-hybrides-etape1-close-gemm-dense-17-09 : Nemotron calibA PPL
        ...
        # Sage (a6a7436, 15/09 ; gate!=up 7c3698d, 16/09) : l'echelle AWQ par
        # expert reste dans la pile -- le moteur (Laurine) la porte cote
        ...
        # ambiguite d'absence.
        #
        # Sage (`sage-organisation-16-09.md`, Océane `verdict-glm-awq-int8-
        ...
        #
        # Sage (`sage-glm-mma0-verdict-16-09.md` § 2/4) : sous la pile
        ...
        # 16 lui-meme, pas l'alpha -- `hadamard_experts` (Sage, `sage-
        ...
        # sage-p2-qkvo-int8-canal-18-09 : q/k/v/o seuls, et seulement si le
        ...
        # sage-awq-relu2-garde-repli-17-09 : geste (b), repli PAR TENSEUR au
        ...
        # sage-awq-plancher-median-faute-18-09, garde n°2 : l'ÉTENDUE des
        ...
        if candidat_budget:
            ...
            wider = PROMOTE[fmt]
            # sage-p2-qkvo-int8-canal-18-09 : sur Qwen3-Coder, q/k/v/o partent
    ...
    if budget_candidats:
        ...
        #            place A dix fois devant B. Argument de Jerome, VERIFIE sur
        ...
        _GENRE_PRIORITE = {
            ...
            # aucune normalisation qui l absorbe. HYPOTHESE de Jerome, pas
        ...
        def _cle(c):
            ...
            if _mode == "base_croissant":
                ...
                #
                # Nomme par Jerome avant la mesure, et construit tout de suite
        ...
        if _mode == "liste":
            ...
            # TROISIEME GARDE (Jerome) : une liste figee qui se desynchronise
    ...
    if tenseurs_replies:
        ...
        if part > PLAFOND_REPLI:
            ...
            raise ValueError(
                ...
                f"sage-awq-relu2-garde-repli-17-09.")
        ...
        print(f"[acvram] {len(tenseurs_replies)}/{len(ratios_tous)} tenseur(s) "
             ...
             f"nemotron_h — sage-awq-relu2-garde-repli-17-09).", flush=True)
    if ratios_hors_bornes:
        ...
        raise ValueError(
            ...
            f"sage-awq-experts-peu-routes-portee-17-09, motif Nemotron "
